...
// This structure is needed because we only need to check the status
// of a statement handle once as a test. Other checks should not be
...
//===========================================================================================================
	var_list_t *var_list;
	var_list = load_api_vars(_T("SQLAllocateStatement"), charset_file);
...
//======================================================================================
	...
	if (MX_MP_SPECIFIC == MX_SPECIFIC)
		LogMsg(LINEBEFORE+SHORTTIMESTAMP,_T("Begin testing API => MX specific SQLAllocStmt.\n"));
	else
		LogMsg(LINEBEFORE+SHORTTIMESTAMP,_T("Begin testing API => MP specific SQLAllocStmt.\n"));
	...
	/*
	MAY 9, 2014:
	COMMENTING THIS OUT AS CURRENTLY DDLs FAIL WITH AUTOCOMMIT OFF IN TRAF. 
	*/
	/* 6-5-00:
...
// General statement handle exercise testcases.
	...
	for (k = 0; k < 2; k++)
	{
		for (i = 0; i < NUM_STMT; i++)
		{
			...
			_stprintf(Heading,_T("SQLAllocstmt: Test #%d\n"), i);
			...
			if (returncode == SQL_SUCCESS)
			{
				...
				_stprintf(Heading,_T("SQLAllocStmt: create table teststmthndl%d\n"),i+1);
				TESTCASE_BEGINW(Heading);
		...
		for (i = 0; i < NUM_STMT; i++)
		{
			_stprintf(Heading,_T("SQLAllocStmt: inserts on Stmt Handle(%d)\n"),i);
			TESTCASE_BEGINW(Heading);
			if (k == 0)	// prepare & execute 
			{
		...
		for (i = 0; i < NUM_STMT; i++)
		{
			_stprintf(Heading,_T("SQLAllocStmt: selects on Stmt Handle(%d)\n"),i);
			TESTCASE_BEGINW(Heading);
			if (k == 0)	// prepare & execute 
			{
		...
		{
			_stprintf(Heading,_T("SQLAllocStmt: fetches on selects on Stmt Handle(%d)\n"),i);
			TESTCASE_BEGINW(Heading);
		...
		for (i = 0; i < NUM_STMT; i++)
		{
			_stprintf(Heading,_T("SQLAllocStmt: SQLFreeStmt(%d) with SQL_CLOSE\n"),i);
			TESTCASE_BEGINW(Heading);
		...
		for (i = 0; i < NUM_STMT; i++)
		{
			_stprintf(Heading,_T("SQLAllocStmt: SQLFreeStmt(%d) with SQL_DROP\n"),i);
			TESTCASE_BEGINW(Heading);
		...
		_stprintf(Heading,_T("SLQAllocStmt: same sql operations on different of SQLAllocstmt(%d)s\n"),i);
		TESTCASE_BEGINW(Heading);
 		for (i = 0; i < NUM_STMT; i++)
		{
			hstmt[i] = (SQLHANDLE)pTestInfo->hstmt;
...
// Asynchronus Testcases
	...
	if (MX_MP_SPECIFIC == MX_SPECIFIC)
	{
		for (i = 0; i < NUM_ASYNC_STMT; i++)
		{
			...
			_stprintf(Heading,_T("SQLAllocstmt: Test #%d and Enable the ASYNC MODE ON.\n"),i);
			TESTCASE_BEGINW(Heading);
			returncode = SQLAllocStmt((SQLHANDLE)hdbc, &hstmt[i]);
			if(!CHECKRC(SQL_SUCCESS,returncode,"SQLAllocStmt"))
			{
				...
				for (i = 0; i < NUM_ASYNC_STMT; i++)
				{
					if (AsyncStmt[i].status == SQL_STILL_EXECUTING)
					{
						...
						if (AsyncStmt[i].checked)
						{
							...
							{
								_stprintf(Heading,_T("SQLAllocStmt: Result Test: %d; SQLStmt: %s.\n"),i,StmtQueries(AsyncOper[k],teststmthndl[i],StmtHndlStr));
								TESTCASE_BEGINW(Heading);
								if (returncode != SQL_SUCCESS)
								{
									TEST_FAILED;
						...
						else
						{
							...
							_stprintf(Heading,_T("SQLAllocStmt: Status Test: %d; SQLStmt: %s.\n"),i,StmtQueries(AsyncOper[k],teststmthndl[i],StmtHndlStr));
							TESTCASE_BEGINW(Heading);
...
//====================================================================================================
	...
	LogMsg(SHORTTIMESTAMP+LINEAFTER,_T("End testing API => SQLAllocStmt.\n"));
