...
def test002(desc="""a02 add 7 mix columns (start with fix columns"""):
...
#-------------------------------------------------------------------
  ...
  , primary key (orders ASC) not droppable
    ...
    #add column 3: fixed length field, default NULL
    stmt = """alter table a02table add column num1 NUMERIC (9,2) UNSIGNED 
  ...
  91, '91', 9.1);"""
    ...
    stmt = """showddl a2itable;"""
    output = _dci.cmdexec(stmt)

    stmt = """insert into a2itable select * from a02table;"""
    output = _dci.cmdexec(stmt)

    stmt = """select * from a2itable;"""
    output = _dci.cmdexec(stmt)
  ...
  (106,'A',1.06,'A',106,'VVV_VVV',1060,'66',10.06, 106.00,'a');"""
    ...
    # expected 25 rows
    stmt = """select orders, num10, vch3, vch11 from a2itable order by 1;"""
    output = _dci.cmdexec(stmt)
  ...
  default 'xxxxxxxxxx';"""
    ...
    # Expect to get error 8101
    # alter table a02table add column int1 int unsigned
    # constraint int_a02 check (int1 < 10000);
    stmt = """alter table a2itable add constraint int_a2i check (int1 < 10000);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    ...
    stmt = """select orders, vch13, vch2 from a2itable where orders = 61 order by orders;"""
    output = _dci.cmdexec(stmt)
...
order by orders;"""
  ...
  where orders = 123; """
    ...
    # -------------------------------------------------------------------
    # add column 17: variable length field, default NULL
    # -------------------------------------------------------------------
...
;"""
  ...
  constraint int3_c primary key asc not null;"""
   ...
   default 'aaaaa' heading '';"""
    ...
    _testmgr.testcase_end(desc)
# -------------------------------------------------------------------
#testcase a04 a04 add 7 mix columns (start with fix columns)
# -------------------------------------------------------------------
...
#   * update a range of add columns
# -------------------------------------------------------------------
def test004(desc="""a04 add 7 mix columns (start with fix columns)"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
    ...
    vch1 CHARACTER(3) not null not droppable
  , vch2 CHAR (3)
)
store by (vch1);"""
    output = _dci.cmdexec(stmt)
   ...
   default null heading 'LargeInt';"""
    ...
    # a04.9:
    stmt = """insert into a04table (vch1, vch2, int3) values ('88','88',8);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output) 

    # a04.10:    
    stmt = """select * from a04table order by vch1;"""
    output = _dci.cmdexec(stmt)
   ...
   default 'aaaaa' heading '';"""
    ...
    # a04.15:
    stmt = """insert into a04table (vch1, int3, vch5) values ('10', 10, 'Var10');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    ...
    # a04.18:
    stmt = """insert into a04table (vch1, int3, vch5) values ('12',12,'Var12');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    ...
    # a04.20:
    stmt = """insert into a04table (vch1, int3) values ('14',14);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    
    # a04.21:
    # ERROR[4033] column VCH1 is a primary key or clustering key column and
    #   cannot be updated
    ...
    stmt = """insert into a04table (vch1, int3, vch6) values ('15', 15,'V15');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    ...
    stmt = """insert into a04table (vch1, int3, pic7) values ('17',17, 17.17);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a04table (vch1, int3, pic8) values ('18',18, 18.18);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)    
  ...
  where int3 < 100 and i64_4 is NULL;"""
    ...
    stmt = """insert into a04table (vch1, int3, pic7) values ('19',19, 19.19);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
...
# -------------------------------------------------------------------
#testcase test005 a05 add 7 mix columns: date, time, timestamp ...
# -------------------------------------------------------------------
# Simple add columne test
...
# -------------------------------------------------------------------
def test005(desc="""a05 add 7 mix columns: date, time, timestamp ..."""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
...
;"""
  ...
  (16, 'row_16', date '2000-01-31', time '08:00:00', 'noe.smith',
   ...
   timestamp '2000-02-28:08:08:08','');"""
    ...
    # -------------------------------------------------------------------
    # add column 9: usrs varchar(20) default ''
    # -------------------------------------------------------------------
  ...
  where dt00 = date '2005-01-01' and
        ...
        tsdf = timestamp '0001-01-01:12:00:00.000000' and usrs = '';"""
    output = _dci.cmdexec(stmt)
...
;"""
  ...
  (22, time '03:33:00.1', time '03:33:00.12', 
    ...
    #dbug1
    stmt = """select int1, usrs, dt00 from a5itable order by int1;"""
    output = _dci.cmdexec(stmt)

    _testmgr.testcase_end(desc)

    # -------------------------------------------------------------------
    #testcase a06 a06 add 13 mix columns: date, time, timestamp, interval
    # -------------------------------------------------------------------
    # Simple add columne test
    ...
    # Add fixed columns and variable columns in sequence
    # -------------------------------------------------------------------
def test006(desc="""a06 add 13 mix columns: date, time, timestamp, interval"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
    ...
    output = _dci.cmdexec(stmt)
    stmt = """drop view a6_view3;"""
    output = _dci.cmdexec(stmt)
    stmt = """drop view a6_view4;"""
    output = _dci.cmdexec(stmt)
    stmt = """drop view a6_view5;"""
    output = _dci.cmdexec(stmt)
    stmt = """drop table a06table;"""
    output = _dci.cmdexec(stmt)
    
    # # a06.1: table start with 2 fixed length field
...
;"""
    ...
    # a06.2: insert couple rows 
    # row 1 and 2
    stmt = """insert into a06table (orders) values (001);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a06table values (002, DATE '2001-01-02', 
    ...
    # -------------------------------------------------------------------
    #  a06.4: add column 5: s12_2 INTERVAL SECOND(12,2);
    # -------------------------------------------------------------------
    ...
    # a06.5: row 3 and 4
    stmt = """insert into a06table (orders, dt00) values (003, date '2005-01-01'); """
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a06table (orders, s12_2) 
  values (004, interval - '10.0' second(12,2));"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    # -------------------------------------------------------------------
    # a06.6: add column 6: yr4 INTERVAL YEAR(4) 
    # -------------------------------------------------------------------
    ...
    # a06.7: add row 5, 6 and 7
    stmt = """insert into a06table values 
  (005, date '2005-01-01', time '15:00:00', timestamp '2005-01-01:15:00:00',
   interval '5.5' second(12,2), interval '1005' year(4));"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a06table values 
  (006, date '2006-01-01', time '16:00:00', timestamp '2006-01-01:16:00:00',
   interval '6000.55' second(12,2), interval '1006' year(4));"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a06table (orders, ti00, s12_2)values 
  (007, time '17:00:00', interval '7000.55' second(12,2));"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    # -------------------------------------------------------------------
    # a06.8: add column 7: mo00 INTERVAL MONTH default interval '0' month
    # -------------------------------------------------------------------
  ...
  (008, interval '7.2' second(12,2), interval '1717' year(4));"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    
    stmt = """insert into a06table values 
  (009, date '2006-09-09', time '19:00:00', timestamp '2009-01-01:19:00:00',
   interval '9000.55' second(12,2), interval '1009' year(4),
   interval '29' month);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    
    # a06.10: expected 9 rows selected
    stmt = """select * from a06table order by orders;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a06exp""", 'a06s5')
    # -------------------------------------------------------------------
    # a06.11: add column 8: vchar8 VARCHAR(8) default 'interval'
    # -------------------------------------------------------------------
    ...
    # a06.12:
    stmt = """create view a6_view3 as select
  orders, dt00, ti00, ts00, s12_2, yr4, mo00 
    from a06table;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)

    stmt = """select * from a6_view3 order by orders;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a06exp""", 'a06s6')
    
    # a06.13: insert row 10 and 11
    stmt = """insert into a06table (orders, mo00) values (010, interval + '10' month);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    
    stmt = """insert into a06table values 
  (011, date '2006-10-10', time '10:10:00', timestamp '2009-10-10:10:10:10',
   interval '10.10' second(12,2), interval + '10' year(4),
   interval - '2' month, 'row_11');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    
    # a06.14: expected 11 rows selected
    stmt = """select * from a06table order by orders;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a06exp""", 'a06s7')
    
    # -------------------------------------------------------------------
    # a06.15: add column 9: day18 INTERVAL day(18)
    # -------------------------------------------------------------------
  ...
  (012, date '1996-01-31', timestamp '1996-01-31:12:12:12',
       interval '1212.12' second(12,2), interval '12' month);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    
    stmt = """insert into a06table values
  (013, date '2006-10-13', time '13:13:03', timestamp '2003-10-13:13:10:10',
   ...
   interval + '2' month, 'row_13',interval '13' day);"""
    ...
    # a06.18: orders=004 and 011 got updated to current_date and current_timestamp
   ...
   where s12_2 = interval - '10.00' second(12,2)
    ...
    # -------------------------------------------------------------------
    # a06.20: add column 10: mo_mo interval month to month
    # -------------------------------------------------------------------
  ...
  default interval '11' month to month;"""
    ...
    # -------------------------------------------------------------------
    # a06.21: add column 11: d07_s5 interval day(7) to second(5)
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # a06.22: add column 12: h08_s0 interval hour(8) to second(0)
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # a06.23: add column 13: m10_s interval minute(10) to second
    # -------------------------------------------------------------------
  ...
  orders, s12_2, mo_mo, h08_s0, vchar8 from a06table 
   where orders >= 010;"""
    ...
    # a06.25:
    stmt = """create view a6_view4 as select
  orders, day18, mo_mo, d07_s5, h08_s0, vchar8 from a06table;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    ...
    # -------------------------------------------------------------------
    # a06.26: add column 14: y11_m interval year(11) to month
    # -------------------------------------------------------------------
  ...
  (020, interval '1414-01' year(11) to month);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    # -------------------------------------------------------------------
    # a06.27: add column 15: s5f1 interval second(5,1)
    # -------------------------------------------------------------------
  ...
  (021, interval '2121.21' second(12,2), 
         interval '100000-11' year(11) to month);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    # -------------------------------------------------------------------
    # a06.28: add column 16: h02_s6 interval hour(2) to second(6)
    # -------------------------------------------------------------------
  ...
  (023, interval '23232.3' second(5,1), 
    ...
    # a06.30:
    stmt = """create view a6_view5 as select
  orders, m10_s, y11_m, s5f1, h02_s6, vchar3 from a06table;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    ...
    stmt = """select * from a6_view4 order by orders;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a06exp""", 'a06s10')

    stmt = """select * from a6_view5 order by orders;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a06exp""", 'a06s11')
    
    # a06.31
    stmt = """update a06table set s5f1 = interval '12345.0' second(5,1) where orders < 010;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_updated_msg(output,9)
  ...
  (017, date '1991-07-31', time '17:30:30', 
    ...
    stmt = """select * from a06table where orders = 017;"""
    output = _dci.cmdexec(stmt)
    ...
    # a06.37:
    stmt = """delete from a06table where y11_m is NULL and orders <= 005;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_deleted_msg(output,3)
    ...
    stmt = """select * from a06table where 
  m10_s = interval '16:16.123456' minute(10) to second;"""
    output = _dci.cmdexec(stmt)
   
    stmt = """delete from a06table where 
  m10_s = interval '16:16.123456' minute(10) to second;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_deleted_msg(output,1)
    time.sleep(5)
    
    stmt = """select * from a06table where orders > 20;"""
    output = _dci.cmdexec(stmt)
    
    stmt = """update a06table set mo_mo = interval '22' month to month where
  orders > 20;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_updated_msg(output,3)
    time.sleep(5)
    
    stmt = """select * from a06table where orders = 10;"""
    output = _dci.cmdexec(stmt)
    
    stmt = """update a06table set vchar8 = 'vchar8'
   where orders = 10;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_updated_msg(output,1)
    time.sleep(15)
    
    stmt = """select orders, m10_s, mo_mo, vchar8 from a06table order by 1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a06exp""", 'a05s21')
    time.sleep(5)
    ...
    # -------------------------------------------------------------------
    #testcase a07 a07 verify missing data after add variable column
    # -------------------------------------------------------------------
    # same as addt003/a09 test except table starts with all fix columns
    # -------------------------------------------------------------------
def test007(desc="""a07 verify missing data after add variable column"""):
    ...
    # # a07.1: table starts with fixed length columns only
...
store by primary key;"""
    ...
    stmt = """insert into a07table values (1,1,'1');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    # -------------------------------------------------------------------
    #  a07.2: add fixed length column, no default
    # -------------------------------------------------------------------
    stmt = """alter table a07table add column int3 int unsigned;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    
    stmt = """insert into a07table values (2,2,'2',2);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table (int1, sma2) values (3, 3);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table values (4,4,'4',4);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    # -------------------------------------------------------------------
    # a07.3: add fixed length column, default literal
    # -------------------------------------------------------------------
    stmt = """alter table a07table add column int4 int default 3000;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)

    stmt = """insert into a07table (int1, int3) values (5,5);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table values (6,6,'6',6,6);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    # -------------------------------------------------------------------
    # a07.4: add fixed length column, no default
    # -------------------------------------------------------------------
    stmt = """alter table a07table add lgt5 largeint;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)

    stmt = """insert into a07table values (7,7,'7',7,7,7);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table values (8,8,'8',8,8,8);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table (int1, int4) values (9,9);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table (int1, sma2, int3) values (10,10,10);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    # -------------------------------------------------------------------
    # a07.5: add fixed length column, default literal
    # -------------------------------------------------------------------
    stmt = """alter table a07table add column num6 numeric(7,2) default 6000;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)

    stmt = """insert into a07table (int1, int3, int4) values (11,11,11);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table values (12,12,'12',12,12,12,12);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table (int1, sma2, lgt5) values (13,13,13);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table values (14,14,'14',14,14,14,14);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table (int1, lgt5, num6) values (15,15,15);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    
    # a07.6:
    stmt = """select * from a07table;"""
    output = _dci.cmdexec(stmt)
    
    # -------------------------------------------------------------------
    # a07.7: add fixed length column, default current_user
    # -------------------------------------------------------------------
    stmt = """alter table a07table add usr7 char(16) default '';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)

    stmt = """insert into a07table (int1, sma2, int4) values (16,16,16);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table (int1, int3, usr7) values (17,17,'usr17');"""
    output = _dci.cmdexec(stmt)
    
    # -------------------------------------------------------------------
    # a07.8: add fixed length column, default current_date
    # -------------------------------------------------------------------
    stmt = """alter table a07table add dt08 date default current_date;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)

    stmt = """insert into a07table (int1, int4, num6) values (18,18,18);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table values (19,19,'19',19,19,19,19,'usr19',date '2000-12-31');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table (int1, int3, num6, usr7) values 
  (20,20,20,'usr20');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    
    stmt = """select int1, sma2, xch2, int3, int4, lgt5, num6 from a07table
  order by 1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a07exp""", 'a07s08')
     
    # -------------------------------------------------------------------
    # a07.9: add fixed length column, default current_time
    # -------------------------------------------------------------------
    stmt = """alter table a07table add ti09 time default current_time;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)

    stmt = """insert into a07table (int1, int4, usr7, dt08) values
  (21,21,'usr21', date '2021-12-31');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)

    stmt = """insert into a07table values (22,22,'22',22,22,22,22,'usr22',
  date '2022-12-31', time '22:22:22');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    # -------------------------------------------------------------------
    # a07.10: add fixed length column, default current_time
    # -------------------------------------------------------------------
    stmt = """alter table a07table add ts10 timestamp default current_timestamp;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
  ...
  (23,23,'usr23', date '2023-12-31');"""
    ...
    stmt = """insert into a07table values (24,24,'24',24,24,24,24,'usr24',
  date '2024-12-31', time '20:24:24', timestamp '2024-12-31:20:24:24');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output)
    
    # -------------------------------------------------------------------
    # a07.11: add fixed length column, default current_date
    # -------------------------------------------------------------------
    stmt = """alter table a07table add vch11 varchar(1) default null;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
  ...
  order by 1;"""
    ...
    # -------------------------------------------------------------------
    # int1=18,20 where dt08 = current_date
    # int1=21,23 where ti09 = current_time
    # int1=23    where ts10 = current_timestamp
    # -------------------------------------------------------------------
    # a07.12a:
    stmt = """select int1, usr7, dt08, ti09, ts10, vch11 from a07table
  where int1 in (21,23,18,20) order by 1;"""
    output = _dci.cmdexec(stmt)
    #_dci.expect_file(output, defs.test_dir + """/a07exp""", 'a07s12a')

    # a07.12b:
    stmt = """select int1, usr7, dt08, ti09, ts10, vch11 from a07table
  where int1 not in (21,23,18,20) order by int1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a07exp""", 'a07s12b')

    # a07.13:
    stmt = """select * from a07table where int1 = 15 order by int1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a07exp""", 'a07s13')
    
    # a07.14:
    stmt = """select * from a07table where int1 < 10 order by int1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a07exp""", 'a07s14')

    # a07.15:
    stmt = """select * from a07table where int3 is null order by int1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a07exp""", 'a07s15')
    
    # a07.16:
    stmt = """select * from a07table where lgt5 is not null order by int1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a07exp""", 'a07s16')
    
    # a07.17:
    stmt = """update a07table set num6 = int1 where int1 =10;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_updated_msg(output)
    
    # a07.18:
    stmt = """update a07table set usr7 = 'LEO',
                    ti09 = time '19:19:19' where int1 < 5;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_updated_msg(output,4)
    
    # a07.19:
    stmt = """update a07table set dt08 = date '1999-12-31', 
   ...
   where int1 > 3 and int1 < 10;"""
    ...
    # a07.20:
    stmt = """update a07table set int4 = int4 + 2000,
  ...
  order by 1;"""
    ...
    # a07.22a:
    stmt = """select int1, usr7, dt08, ti09, ts10, vch11 from a07table
  where int1 not in (21,23,18,20) order by 1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a07exp""", 'a07s22a')

    # a07.22b:
    stmt = """select int1, usr7, dt08, ti09, ts10, vch11 from a07table
  where int1 in (21,23,18,20) order by 1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a07exp""", 'a07s22b')
    
    #expectfile $test_dir/a07exp a07s22b
    #sh diff ${work_dir}/a07s12log ${work_dir}/a07s22log

    #showddl a07table;
    stmt = """showddl a07table;"""
    output = _dci.cmdexec(stmt)

    _testmgr.testcase_end(desc)

    # -------------------------------------------------------------------
    #testcase a08 a08 add mix columns
    # -------------------------------------------------------------------
    # Simple add columne test
    ...
    #   column 9: dec1     dec(9,2) unsigned
    # -------------------------------------------------------------------
def test008(desc="""a08 add mix columns"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
  ...
  ;"""
    ...
    stmt = """insert into a08table values (1,'a'), (2,'b'), (3, 'c');"""
    output = _dci.cmdexec(stmt)
  ...
  default null no heading;"""
    ...
    stmt = """select * from a08table order by 1;"""
    output = _dci.cmdexec(stmt)
  ...
  heading 'SmallInt';"""
   ...
   values (63, 5.03, 24, 'variable length column vch2');"""
    ...
    stmt = """select orders, sma1, vch1, vch2 from a08table order by 1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a08exp""", 'a08s04')
    time.sleep(5)
    
    stmt = """select orders, sma1, vch1, vch2 from a08table order by 1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a08exp""", 'a08s05')
    # -------------------------------------------------------------------
    # add column 7: fixed length field, default NULL
    # -------------------------------------------------------------------
  ...
  values (81,8.08,81,81);"""
   ...
   'Bill and Joe ran softly near one of the hill. Some birds sprinted over Madelyn! Racoons and cats and blue birds. Oh My! Violet and Fred rushed sadly towards some door! Genelle and Janine split steadily across one of the string???',
    ...
    stmt = """select orders, num1, vch3, int1 from a08table order by 1;"""
    output = _dci.cmdexec(stmt)
    time.sleep(5)
  ...
  91, '91', 9.1);"""
    ...
    stmt = """select orders, vch2, int1 from a08table where ch1 is NULL and int1 > 300;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a08exp""", 'a08s13')
    
    stmt = """select orders, vch2, int1 from a08table where dec1 is NOT NULL order by 1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a08exp""", 'a08s14')
    
    stmt = """select orders, vch3 from a08table where dec1 < 10 order by 1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a08exp""", 'a08s15')
    
    stmt = """select orders, int1, sma1, vch1, vch2, vch3 from a08table where vch3 = 'AB'
  order by 1;"""
    output = _dci.cmdexec(stmt)
    ...
    #stmt = """select * from a08table where orders < 20 order by 1;"""
    #output = _dci.cmdexec(stmt)
  ...
  values (102,'A','VCH6',1.02);"""
    ...
    stmt = """select * from a08table where vch3 = 'EE';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output,0)
    
    # fetch a range of rows (subset fetch)
    stmt = """select * from a08table where num10 < 2.00;"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """select orders, num10, vch3, dec1 from a08table;"""
    output = _dci.cmdexec(stmt)
  ...
  num1 = ?i02, sma1 = ?i02, int1 = ?i02, dec1 = ?i02 where orders = 1;"""
    ...
    stmt = """select orders, vch11, num1, sma1, int1, dec1 from a08table where orders < 3
  order by 1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a08exp""", 'a08s26')

    stmt = """select orders, vch3, num10, vch11, dec1 from a08table order by 1;"""
    output = _dci.cmdexec(stmt)
  ...
  default 'xxx';"""
    ...
    stmt = """select orders, vch13, vch2 from a08table where orders = 61;"""
    output = _dci.cmdexec(stmt)
  ...
  where orders < 50 or orders > 100;"""
    ...
    stmt = """select orders, int14, sma12 from a08table where orders between 50 and 100
order by 1;"""
    output = _dci.cmdexec(stmt)
  ...
  values (120,1.20,'V8',120, 'y');"""
   ...
   values (121,'X','x','vch2_varchar255','xx','x');"""
    ...
    stmt = """select orders, vch15, ch1 from a08table where ch1 in ('8','A','y');"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """select orders, vch15, ch1 from a08table order by 1;"""
    output = _dci.cmdexec(stmt)
  ...
  (124,'B',12.4,'b',124,'VCH6_COL16',1240,'Bb',1.24,
   11.24,'b',1124,'VCH13_COL16',11240,'bB',12.1234567);"""
    ...
    stmt = """select orders, num1, vch3, sma12, vch15 from a08table where ch1 = '8' order by 1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a08exp""", 'a08s29')
    
    stmt = """select orders, int1, sma1, vch11, vch13, vch15 from a08table where vch1 = 'x'
order by 1;"""
    output = _dci.cmdexec(stmt)
  ...
  where orders = 123;"""
    ...
    stmt = """select * from a08table where orders = 123 order by 1;"""
    output = _dci.cmdexec(stmt)
  ...
  values (128, 'J', 'jj', '12345678901234567890123456789012345678901234567890123456789012345678901234567890');"""
    ...
    stmt = """select orders, dec1, ch1, int1, vch15 from a08table where orders > 123
  order by 1;"""
    output = _dci.cmdexec(stmt)
   ...
   and sma12 = -1 and int14 = -100 and orders > 100;"""
    ...
    stmt = """select orders, sma12, int14, vch17 from a08table where orders > 100;"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """select orders, vch15, vch17 from a08table where orders < 5 order by 1;"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """select orders, vch15, vch17 from a08table;"""
    output = _dci.cmdexec(stmt)
