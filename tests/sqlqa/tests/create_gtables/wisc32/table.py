...
def create_and_load(hptestmgr, prop_file, name, files, count_str, delim):
    global _testmgr
    global _dci
    ...
    # update stats
    stmt = 'update statistics for table ' + table + ' on every column sample random 5 percent;'
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_str_token(output, count_str)
