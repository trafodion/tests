...
##testcase TPCDS1 TPCDS Query 1 variant
#== object customer_store_return doesn't exist, remove test
#== references the non-existing object
...
#
#$SQL_updated_msg 1
#update $my_schema.timetable set T2_cmp_time = time '$et' where query = ?q
#;
#
#$SQL_updated_msg 1
#update $my_schema.timetable set T2_cost =
...
#
#$SQL_updated_msg 1
#update $my_schema.timetable set T2_time = time '$et'
#where query = ?q;
#
#$SQL_updated_msg 1
#execute compare_T1_and_T2;
#
#$SQL_updated_msg 1
#execute get_low_values;
#
##expect any *PASS*
#execute comp_plan_and_cost;
#
#select * from $my_schema.timetable
# where query = ?q;
#
##endtestcase
...
def test001(desc="""TPCDS Query 16 variant"""):
    global _testmgr
    global _testlist
    global _dci
    ...
    stmt = """set param ?q 16;"""
    output = _dci.cmdexec(stmt)
    stmt = """control query default scratch_max_opens_sort '3';"""
    output = _dci.cmdexec(stmt)
...
def test002(desc="""TPCDS Query 34"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
    stmt = """set param ?q 34;"""
    output = _dci.cmdexec(stmt)
    stmt = """control query default scratch_max_opens_sort '1';"""
    output = _dci.cmdexec(stmt)
    #set showshape on;
...
def test003(desc="""TPCDS Query 40"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
...
def test004(desc="""TPCDS Query 50 variant"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
    ...
    stmt = """set param ?q 50;"""
    output = _dci.cmdexec(stmt)
    stmt = """control query default scratch_max_opens_sort '2';"""
    output = _dci.cmdexec(stmt)
...
def test005(desc="""TPCDS Query 46 variant"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
