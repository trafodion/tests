...
where query = ?q ;"""
    ...
    #update """ + defs.my_schema + """.timetable set bp_vs_T2_plan =
    #(select case
    #         when plan_ops.match_ops =  bp_vs_T2.match_ops then 'MATCH'
    #         else 'DIFFER'
    #       end
    #from (select count(*) match_ops
    #      from """ + defs.my_schema + """.plantable plan_bp,
    #           """ + defs.my_schema + """.plantable plan_T2
    ...
    #     (select max(seq_num) match_ops from """ + defs.my_schema + """.plantable plan_bp
