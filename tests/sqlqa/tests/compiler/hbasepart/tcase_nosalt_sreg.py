...
def qry_orders_nosaltsreg(cqd_hash2p_val, cqd_rangep_val, stats_flag=None):
    global _testmgr
    global _testlist
    global _dci

    if stats_flag == 'stats':
        tblname = 'nosaltsreg_orders_ws'
    else:
        tblname = 'nosaltsreg_orders'
    ...
    # --Q8------------------------------------------------
    # query involving small range on keys: parallel plan
    stmt = ("""prepare XX from select * from """ + tblname +
...
def qry_lineitem_nosaltsreg(cqd_hash2p_val, cqd_rangep_val, stats_flag=None):
    global _testmgr
    global _testlist
    global _dci

    if stats_flag == 'stats':
        tblname = 'nosaltsreg_lineitem_ws'
    else:
        tblname = 'nosaltsreg_lineitem'
    ...
    stmt = """explain options 'f' XX;"""
    output = _dci.cmdexec(stmt)
    #_dci.expect_file(output, defs.test_dir + expfile, 'p4' + sflag)
    ...
    stmt = """explain options 'f' XX;"""
    output = _dci.cmdexec(stmt)
    #_dci.expect_file(output, defs.test_dir + expfile, 'p5' + sflag)
...
# ===================================================
# TESTCASES for NONSALTED,SINGLE-REGION table WITH STATS
# hash2 part = ON, range part = ON: serial plan
# hash2 part = ON, range part = OFF: serial plan
# hash2 part = OFF, range part = ON: serial plan
...
# ===================================================
def test_hpON_rpON_nosaltsreg(desc="""hpON,rpON,stats,nosalt,single-region"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist):
        return
    defs.testid = """test_hpON_rpON_nosaltsreg"""

    # hash2 partitioning = ON, range partitioning = ON
    # nonsalted, single region table with statistics
    # expect: serial plan
    qry_orders_nosaltsreg('ON', 'ON', 'stats')
    qry_lineitem_nosaltsreg('ON', 'ON', 'stats')
    _testmgr.testcase_end(desc)


def test_hpON_rpOFF_nosaltsreg(desc="""hpON,rpOFF,stats,nosalt,
single-region"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist):
        return
    defs.testid = """test_hpON_rpOFF_nosaltsreg"""

    # hash2 partitioning = ON, range partitioning = OFF
    # nonsalted, single region table with statistics
    # expect: serial plan
    qry_orders_nosaltsreg('ON', 'OFF', 'stats')
    qry_lineitem_nosaltsreg('ON', 'OFF', 'stats')
    _testmgr.testcase_end(desc)


def test_hpOFF_rpON_nosaltsreg(desc="""hpOFF,rpON,stats,nosalt,
single-region"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist):
        return
    defs.testid = """test_hpOFF_rpON_nosaltsreg"""

    # hash2 partitioning = OFF, range partitioning = ON
    # nonsalted, single region table with statistics
    # expect: serial plan
    qry_orders_nosaltsreg('OFF', 'ON', 'stats')
    qry_lineitem_nosaltsreg('OFF', 'ON', 'stats')
    _testmgr.testcase_end(desc)


def test_hpOFF_rpOFF_nosaltsreg(desc="""hpOFF,rpOFF,stats,nosalt,
single-region"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist):
        return
    defs.testid = """test_hpOFF_rpOFF_nosaltsreg"""

    # hash2 partitioning = OFF, range partitioning = OFF
    # nonsalted, single region table with statistics
    # expect: serial plan
    qry_orders_nosaltsreg('OFF', 'OFF', 'stats')
    qry_lineitem_nosaltsreg('OFF', 'OFF', 'stats')
    _testmgr.testcase_end(desc)


# ===================================================
# TESTCASES for NONSALTED,SINGLE-REGION table with NO STATS
# hash2 part = ON, range part = ON: serial plan
# hash2 part = ON, range part = OFF: serial plan
# hash2 part = OFF, range part = ON: serial plan
# hash2 part = OFF, range part = OFF: serial plan
...
# co-partitioned (eg. no repartition/exchange on either side of join).
# ===================================================
def test_hpON_rpON_nostat_nosaltsreg(desc="""hpON,rpON,nostats,nosalt,
single-region"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist):
        return
    defs.testid = """test_hpON_rpON_nostat_nosaltsreg"""

    # hash2 partitioning = ON, range partitioning = ON
    # non-salted, single-region table with NO statistics
    qry_orders_nosaltsreg('ON', 'ON')
    qry_lineitem_nosaltsreg('ON', 'ON')
    _testmgr.testcase_end(desc)


def test_hpON_rpOFF_nostat_nosaltsreg(desc="""hpON,rpOFF,nostats,nosalt,
single-region"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist):
        return
    defs.testid = """test_hpON_rpOFF_nostat_nosaltsreg"""

    # hash2 partitioning = ON, range partitioning = OFF
    # non-salted, single-region table with NO statistics
    qry_orders_nosaltsreg('ON', 'OFF')
    qry_lineitem_nosaltsreg('ON', 'OFF')
    _testmgr.testcase_end(desc)


def test_hpOFF_rpON_nostat_nosaltsreg(desc="""hash2p OFF,rangep ON,nonsalt,
single-region"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist):
        return
    defs.testid = """test_hpOFF_rpON"""

    # range partitioning = ON, with NO statistics
    qry_orders_nosaltsreg('OFF', 'ON')
    qry_lineitem_nosaltsreg('OFF', 'ON')
    _testmgr.testcase_end(desc)


def test_hpOFF_rpOFF_nostat_nosaltsreg(desc="""hash2p OFF,rangep OFF,nonsalt,
single-region"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist):
        return
    defs.testid = """test_hpOFF_rpOFF"""

    # range partitioning = ON, with NO statistics
    qry_orders_nosaltsreg('OFF', 'OFF')
    qry_lineitem_nosaltsreg('OFF', 'OFF')
    _testmgr.testcase_end(desc)
