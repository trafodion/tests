...
and o_totalprice > 59450;"""
    ...
    stmt = """execute explainit;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output)
    
    stmt = """execute explainit2;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output)
    ...
    stmt = """select 'FAIL', operator, detail_stat
...
and
locate('IO_SEC', detail_stat) = 0
and
locate('IO_RAND', detail_stat) = 0;;"""
    output = _dci.cmdexec(stmt)
    
    stmt = """cqd EXPLAIN_DISPLAY_FORMAT 'external_detailed';"""
    output = _dci.cmdexec(stmt)
    _testmgr.testcase_end(desc)
...
l_linestatus;"""
    ...
    stmt = """cqd EXPLAIN_DISPLAY_FORMAT 'internal';"""
    output = _dci.cmdexec(stmt)
...
o_orderpriority,
min(o_totalprice) over (order by o_custkey, o_orderkey
rows between 2456 following and 3456 following    ) as ordercount
from
orders
where
o_orderdate >= date '1993-10-01'
...
o_totalprice,
sum(l_quantity) over (order by l_orderkey
rows between current row and 1800 following) as lquantity    
