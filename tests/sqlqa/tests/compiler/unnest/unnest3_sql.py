...
order by 1 desc;"""
    ...
    ##define unnest3_Q1
    stmt = """PREPARE XX FROM SELECT A  FROM T1  WHERE  B IN
...
(SELECT  T5.R FROM T5 WHERE T5.O > T4.J AND T5.P > T4.K)
    ...
    ##define unnest3_Q2
    stmt = """PREPARE XX FROM SELECT A  FROM T1  WHERE  B NOT IN
...
(SELECT  T5.R FROM T5 WHERE T5.O > T4.J AND T5.P > T4.K   )
    ...
    stmt = """execute XX;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/unnest3.exp""", """Q2_""" + defs.test_id + """""")
    
    ##define unnest3_Q3
    stmt = """PREPARE XX FROM SELECT A  FROM T1  WHERE  B IN
(SELECT  MAX (T2.C) FROM T2  WHERE  T2.D = T1.A OR   T2.E  = 10    OR   EXISTS
...
(SELECT  T5.R FROM T5 WHERE T5.O > T4.J AND T5.P > T4.K   )
    ...
    ##define unnest3_Q4
    stmt = """PREPARE XX FROM SELECT A  FROM T1  WHERE  B NOT IN
(SELECT  MAX (T2.C) FROM T2  WHERE  T2.D = T1.A OR   T2.E  = 10    OR   NOT EXISTS
(SELECT MIN(T3.F), AVG(T3.G) FROM T3  WHERE   (T3.H < T2. E  AND  T3.H > T1.A) AND T3.G   <> ANY
(SELECT  T4.L  FROM T4  WHERE  (T4.J  > T3.G  AND  T3.H < T2. E  AND  T4.K > T2.C  AND  T4.K <  T1.A )  OR  T4. K  = ANY
(SELECT  T5.R FROM T5 WHERE T5.O > T4.J AND T5.P > T4.K   )
))) Order By A;"""
    ...
    stmt = """execute XX;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/unnest3.exp""", """Q4_""" + defs.test_id + """""")
