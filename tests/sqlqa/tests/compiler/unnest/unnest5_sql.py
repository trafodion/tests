...
order by 1 desc;"""
    ...
    ##define unnest5_Q1
    stmt = """PREPARE XX FROM SELECT A , B  FROM T1  WHERE  B <= ALL
...
ORDER BY b,A;"""
    ...
    stmt = """execute XX;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/unnest5.exp""", """Q1_""" + defs.test_id + """""")
...
Order By 2,1;"""
    ...
    stmt = """execute XX;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/unnest5.exp""", """Q2_""" + defs.test_id + """""")
...
Order By 2;"""
    ...
    stmt = """execute XX;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/unnest5.exp""", """Q3_""" + defs.test_id + """""")
