...
def _init(hptestmgr, testlist=[]):
    ...
    _dci.setup_schema(defs.my_schema1)
    _dci.setup_schema(defs.my_schema0)
    _dci.setup_schema(defs.my_schema)   
    _dbrootdci = _testmgr.get_dbroot_dci_proc()
    _dbrootdci.setup_schema(defs.my_schema)

    # Need DB_ROOT to grant the manage_library privage so that the user
    # can create the library
