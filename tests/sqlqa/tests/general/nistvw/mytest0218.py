...
FROM  VWT12;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0218.exp""", """s1""")
...
ORDER BY COL6 DESC;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0218.exp""", """s2""")
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0218.exp""", """s3""")
    # pass if count = 0
