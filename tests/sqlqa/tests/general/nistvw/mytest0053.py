...
WHERE EMPNUM  NOT LIKE '_36';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0053.exp""", """s1""")
...
WHERE NOT(EMPNUM  LIKE '_36');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0053.exp""", """s2""")
    # PASS:0053 If count = 5?
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0053.exp""", """s3""")
    # pass if count = 5
