...
FROM  VWT12;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0221.exp""", """s1""")
...
ORDER BY COL9,COL8,COL7,COL5,COL4,COL3;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0221.exp""", """s2""")
    ...
    stmt = """select count (*) from VWT12;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0221.exp""", """s3""")
    # pass if count = 0
