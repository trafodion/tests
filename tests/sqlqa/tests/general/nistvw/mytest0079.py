...
VALUES('E7','SONG',50);"""
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0079.exp""", """s1""")
...
group by city;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0079.exp""", """s2""")
...
where city IS NULL;"""
    ...
    stmt = """select count (*) from VWSTAFF;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0079.exp""", """s3""")
