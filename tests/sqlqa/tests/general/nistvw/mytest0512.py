...
FROM VWPROJ, VWWORKS, VWSTAFF
WHERE VWPROJ.PNUM = VWWORKS.PNUM
AND VWWORKS.EMPNUM = VWSTAFF.EMPNUM
AND BUDGET - GRADE * HOURS * 100 IN
(-4400, -1000, 4000);"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0512.exp""", """s1""")
...
ORDER BY CITY DESC;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0512.exp""", """s2""")
