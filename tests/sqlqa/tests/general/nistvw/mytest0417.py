...
def _init(hptestmgr, testlist=[]):
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output, 1)
    stmt = """INSERT INTO STAFF1 SELECT * FROM VWSTAFF;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output, 7)
    
    stmt = """SELECT MAX(VWSTAFF1.GRADE), SUM(VWSTAFF1.GRADE)
FROM VWSTAFF1, VWSTAFF
GROUP BY VWSTAFF1.CITY, VWSTAFF.CITY
order by 1, 2;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0417.exp""", """s1""")
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0417.exp""", """s2""")
    # pass if count = 5
    
    stmt = """select count (*) from VWSTAFF1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0417.exp""", """s3""")
    # pass if count = 0
    
    stmt = """select count (*) from staff;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0417.exp""", """s4""")
    # pass if count = 5
