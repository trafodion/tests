...
from VWKK;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0092.exp""", """s1""")
...
where floattest > 123456.123450 and floattest < 123456.123462;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0092.exp""", """s2""")
    ...
    stmt = """select count (*) from VWKK;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0092.exp""", """s3""")
    # pass if count = 0
