...
WHERE INTTEST = 999999999;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0244.exp""", """s1""")
...
WHERE INTTEST = -999999999;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0244.exp""", """s2""")
    ...
    stmt = """select count (*) from VWEE;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0244.exp""", """s3""")
    # pass if count = 0
