...
from VWGG;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0088.exp""", """s1""")
...
where realtest > 1.234561 and realtest < 1.234573;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0088.exp""", """s2""")
    ...
    stmt = """select count (*) from VWGG;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0088.exp""", """s3""")
    # pass if count = 0
