...
FROM  VWT12;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0220.exp""", """s1""")
...
ORDER BY COL7,COL8,COL3,COL4,COL6,COL5 DESC;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0220.exp""", """s2""")
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0220.exp""", """s3""")
    # pass if count = 0
