...
WHERE  EMPNAME LIKE 'Ali%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0229.exp""", """s1""")
...
WHERE  EMPNAME LIKE 'ALI%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0229.exp""", """s2""")
    # PASS:0229 If 1 row is returned and EMPNAME = 'ALICE' (not 'Alice')?
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0229.exp""", """s3""")
    # pass if count = 5
