...
FROM VWVTABLE
GROUP BY COL1
HAVING MAX(COL2) > ANY (SELECT GRADE FROM VWSTAFF)
AND MAX(COL2) < SOME (SELECT HOURS FROM VWWORKS)
ORDER BY COL1;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0259.exp""", """s1""")
    ...
    stmt = """select count (*) from VWVTABLE;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0259.exp""", """s2""")
    # pass if count = 4
