...
WHERE SMALLTEST = 9999;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0243.exp""", """s1""")
...
WHERE SMALLTEST = -9999;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0243.exp""", """s2""")
    ...
    stmt = """select count (*) from VWHH;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0243.exp""", """s3""")
    # pass if count = 0
