...
WHERE C1 = 'ZA' AND C2 = 'ZB';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0247.exp""", """test0247d""")
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0247.exp""", """test0247f""")
    # pass if count = 0
    stmt = """select count (*) FROM T100;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0247.exp""", """test0247g""")
    # pass if count = 0
