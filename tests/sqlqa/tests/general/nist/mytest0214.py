...
'STR66666666666666666666666666666666666666666666666');"""
    ...
    stmt = """select count (*) from t2000;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0214.exp""", """test0214b""")
    stmt = """UPDATE t2000
SET STR140 =
...
from t2000;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0214.exp""", """test0214d""")
    stmt = """UPDATE t2000
SET STR180 =
...
FROM t2000;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0214.exp""", """test0214f""")
    # PASS:0214 If STR110 = ?
    ...
    stmt = """select count (*) from t2000;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0214.exp""", """test0214h""")
    # pass if count = 0
