...
WHERE 24 * 1000 BETWEEN BUDGET - 5000 AND 50000 / 1.7;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0523.exp""", """test0523a""")
...
AND PNUM > 'P2';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0523.exp""", """test0523b""")
...
HAVING 50000 + 2 BETWEEN 33000 AND SUM(BUDGET) - 20;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0523.exp""", """test0523c""")
