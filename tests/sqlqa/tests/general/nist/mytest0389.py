...
def _init(hptestmgr, testlist=[]):
    ...
    ##expectfile ${test_dir}/test0389.exp test0389a
    stmt = """DELETE FROM AA;"""
    output = _dci.cmdexec(stmt)
...
FROM AA;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0389.exp""", """test0389c""")
    ...
    stmt = """select count (*) FROM AA;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0389.exp""", """test0389e""")
    # pass if count = 0
