...
from ss;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0095.exp""", """test0095c""")
    ...
    stmt = """select count (numtest) from ss;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0095.exp""", """test0095e""")
    # pass if count = 0
