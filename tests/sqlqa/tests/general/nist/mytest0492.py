# @@@ START COPYRIGHT @@@
#
# (C) Copyright 2014 Hewlett-Packard Development Company, L.P.
#
#  Licensed under the Apache License, Version 2.0 (the "License");
#  you may not use this file except in compliance with the License.
#  You may obtain a copy of the License at
#
#      http://www.apache.org/licenses/LICENSE-2.0
#
#  Unless required by applicable law or agreed to in writing, software
#  distributed under the License is distributed on an "AS IS" BASIS,
#  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
#  See the License for the specific language governing permissions and
#  limitations under the License.
#
# @@@ END COPYRIGHT @@@

from ...lib import hpdci
from ...lib import gvars
import defs

_testmgr = None
_testlist = []
_dci = None

# TEST:0492 SQLSTATE 22019: data exception/invalid escape char!

def _init(hptestmgr, testlist=[]):
    global _testmgr
    global _testlist
    global _dci
    
    _testmgr = hptestmgr
    _testlist = testlist
    # default hpdci was created using 'SQL' as the proc name.
    # this default instance shows 'SQL>' as the prompt in the log file.
    _dci = _testmgr.get_default_dci_proc()
    
    stmt = """begin work;"""
    output = _dci.cmdexec(stmt)
    
    # setup
    stmt = """UPDATE STAFF
SET CITY = 'Percent%Xunder_'
WHERE EMPNUM = 'E1';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_updated_msg(output, 1)
    stmt = """SELECT COUNT(*)
FROM STAFF
WHERE CITY LIKE '%XX%X_%' ESCAPE 'XX';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_error_msg(output, '8409')
    
    # PASS:0492 If ERROR, data exception/invalid escape char?
    # PASS:0492 0 rows selected OR SQLSTATE = 22019 OR SQLCODE < 0?
    stmt = """SELECT COUNT(*)
FROM STAFF
WHERE CITY LIKE '%XX%X_%' ESCAPE 'X';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0492.exp""", """test0492c""")
    
    # PASS:0492 If count = 1?
    stmt = """SELECT COUNT(*)
FROM STAFF
WHERE CITY LIKE '%XX_%' ESCAPE 'XX';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_error_msg(output, '8409')
    
    # PASS:0492 If ERROR, data exception/invalid escape char?
    # PASS:0492 0 rows selected OR SQLSTATE = 22019 OR SQLCODE < 0?
    stmt = """SELECT COUNT(*)
FROM STAFF WHERE CITY
LIKE '%XX_%' ESCAPE 'X';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0492.exp""", """test0492e""")
    
    # PASS:0492 If count = 1?
    
    # restore
    stmt = """ROLLBACK WORK;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    
    # END TEST >>> 0492 <<< END TEST
    
