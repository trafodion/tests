...
FROM JJ;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0091.exp""", """test0091b""")
...
WHERE FLOATTEST > 12.345672 and FLOATTEST < 12.345684;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0091.exp""", """test0091c""")
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0091.exp""", """test0091e""")
    # pass if count = 0
