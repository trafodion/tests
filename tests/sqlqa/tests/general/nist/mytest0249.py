...
FROM GG;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0249.exp""", """test0249c""")
    # PASS:0249 If REALTEST =  0.1048575 ?
    # PASS:0249 OR  is between 0.1048574 and 0.1048576 ?
    stmt = """SELECT COUNT(*) FROM GG
WHERE REALTEST > 0.1048574 AND REALTEST < 0.1048576;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0249.exp""", """test0249d""")
...
FROM GG;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0249.exp""", """test0249g""")
    # PASS:0249 If REALTEST =  -0.1048575 ?
    # PASS:0249 OR  is between -0.1048576 and -0.1048574 ?
    stmt = """SELECT COUNT(*) FROM GG
WHERE REALTEST > -0.1048576 AND REALTEST < -0.1048574;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0249.exp""", """test0249h""")
    # PASS:0249 If count = 1?
