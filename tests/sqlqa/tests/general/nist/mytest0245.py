...
FROM PP_15;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0245.exp""", """test0245c""")
    # PASS:0245 If NUMTEST = 0.123456789012345?
    stmt = """SELECT COUNT(*) FROM PP_15
WHERE NUMTEST = 0.123456789012345;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0245.exp""", """test0245d""")
...
WHERE NUMTEST = -0.912345678901234;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0245.exp""", """test0245g""")
    # PASS:0245 If count = 1?
