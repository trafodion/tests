...
FROM LL;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0093.exp""", """test0093b""")
...
WHERE NUMTEST > 123456.123450 and NUMTEST < 123456.123462;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0093.exp""", """test0093c""")
    ...
    stmt = """select count (*) FROM LL;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0093.exp""", """test0093e""")
    # pass if count = 0
