...
FROM TEMP_S;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0156.exp""", """test0156b""")
...
FROM TEMP_S;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0156.exp""", """test0156d""")
    # PASS:0156 If count = 10?
    stmt = """SELECT COUNT(DISTINCT EMPNUM)
FROM TEMP_S;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0156.exp""", """test0156e""")
    # PASS:0156 If count = 5?
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0156.exp""", """test0156g""")
    # pass if count = 0
