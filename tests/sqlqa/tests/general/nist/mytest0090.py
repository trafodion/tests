...
FROM II;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0090.exp""", """test0090b""")
...
WHERE DOUBLETEST > 123456.123450 and DOUBLETEST < 123456.123462;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0090.exp""", """test0090c""")
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0090.exp""", """test0090e""")
    # pass if count = 0
