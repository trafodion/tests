...
WHERE  EMPNAME LIKE 'Ali%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0229.exp""", """test0229b""")
    # PASS:0229 If 1 row is returned and EMPNAME = 'Alice' (not 'ALICE')?
    stmt = """SELECT EMPNAME
FROM   STAFF
WHERE  EMPNAME LIKE 'ALI%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0229.exp""", """test0229c""")
    # PASS:0229 If 1 row is returned and EMPNAME = 'ALICE' (not 'Alice')?
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0229.exp""", """test0229e""")
    # pass if count = 5
