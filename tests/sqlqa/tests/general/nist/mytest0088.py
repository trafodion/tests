...
from gg;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0088.exp""", """test0088b""")
...
where realtest > 1.234561 and realtest < 1.234573;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0088.exp""", """test0088c""")
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0088.exp""", """test0088e""")
    # pass if count = 0
