...
WHERE EMPNAME LIKE 'yan____%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0208.exp""", """test0208c""")
    # PASS:0208 If CITY = 'China'?
    stmt = """SELECT CITY
FROM STAFF
WHERE EMPNAME LIKE 'YAN____%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0208.exp""", """test0208d""")
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0208.exp""", """test0208f""")
    # pass if count = 5
