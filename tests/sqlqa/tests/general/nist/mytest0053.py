...
WHERE EMPNUM  NOT LIKE '_36';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0053.exp""", """test0053b""")
    # PASS:0053 If count = 5?
    stmt = """SELECT COUNT(*)
FROM STAFF
WHERE NOT(EMPNUM  LIKE '_36');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0053.exp""", """test0053c""")
    # PASS:0053 If count = 5?
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test0053.exp""", """test0053e""")
    # pass if count = 5
