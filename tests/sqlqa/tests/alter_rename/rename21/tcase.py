...
# A01 ALTER TABLE RENAME (empty, old before new)
# A02 ALTER TABLE RENAME (empty, new before old)
# A03 ALTER TABLE RENAME (non-empty, old before new)
# A04 ALTER TABLE RENAME (non-empty, new before old)
# A05 ALTER TABLE RENAME (large, old before new)
# A06 ALTER TABLE RENAME (large, new before old)
...
def test001(desc="""ALTER TABLE RENAME (empty, old before new)"""):
    global _testmgr
    global _testlist
    global _dci
    ...
    stmt = """alter table tab_emp_original_01 rename to tab_emp_new_01;"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """select * from tab_emp_new_01;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a21exp""", """a01s02""")
    
    _testmgr.testcase_end(desc)

def test002(desc="""ALTER TABLE RENAME (empty, new before old)"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
    ...
    stmt = """alter table tab_emp_original_02 rename to tab_emp_new_02;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    ...
    # insert
    stmt = """insert into tab_emp_new_02 values (1,'eeeeeeee',999999,999999,11,'test_constraint','YZABCDEF',1001,11,'GHIJKLMN');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output, 1)
    stmt = """insert into tab_emp_new_02 values (1,'eeeeeeee',999999,999999,11,'test_constraint','YZABCDEF',1001,11,'GHIJKLMN');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output, 1)
    stmt = """insert into tab_emp_new_02 values (1,'eeeeeeee',999999,999999,11,'test_constraint','YZABCDEF',NULL,NULL,'GHIJKLMN');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_inserted_msg(output, 1)
    stmt = """insert into tab_emp_original_02 values (1,'eeeeeeee',999999,999999,11,'test_constraint','YZABCDEF',1001,11,'GHIJKLMN');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_error_msg(output, '4082')
    stmt = """insert into tab_emp_original_02 values (1,'eeeeeeee',999999,999999,11,'test_constraint','YZABCDEF',1001,11,'GHIJKLMN');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_error_msg(output, '4082')
    stmt = """insert into tab_emp_original_02 values (1,'eeeeeeee',999999,999999,11,'test_constraint','YZABCDEF',NULL,NULL,'GHIJKLMN');"""
    output = _dci.cmdexec(stmt)
    _dci.expect_error_msg(output, '4082')
    ...
    stmt = """select * from tab_emp_new_02;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a21exp""", """a02s02""")
    stmt = """select * from tab_emp_original_02;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_error_msg(output, '4082')
...
def test003(desc="""ALTER TABLE RENAME (non-empty, old before new)"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a21exp""", """a03s01""")
    ...
    stmt = """alter table tab_ten_original_03 rename to tab_ten_new_03;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    ...
    stmt = """select * from tab_ten_new_03;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a21exp""", """a03s02""")
    
    _testmgr.testcase_end(desc)

def test004(desc="""ALTER TABLE RENAME (non-empty, new before old)"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a21exp""", """a04s01""")
    ...
    stmt = """alter table tab_ten_original_04 rename to tab_ten_new_04;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    ...
    stmt = """select * from tab_ten_new_04;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a21exp""", """a04s02""")
    stmt = """select * from tab_ten_original_04;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_error_msg(output, '4082')
...
def test005(desc="""ALTER TABLE RENAME (large, old before new)"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a21exp""", """a05s01""")
    ...
    stmt = """alter table tab_lrg_original_05 rename to tab_lrg_new_05;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    ...
    stmt = """select count(*) from tab_lrg_new_05;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a21exp""", """a05s02""")
    
    _testmgr.testcase_end(desc)

def test006(desc="""ALTER TABLE RENAME (large, new before old)"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a21exp""", """a06s01""")
    ...
    stmt = """alter table tab_lrg_original_06 rename to tab_lrg_new_06;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    ...
    stmt = """select count(*) from tab_lrg_new_06;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a21exp""", """a06s02""")
    stmt = """select count(*) from tab_lrg_original_06;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_error_msg(output, '4082')
