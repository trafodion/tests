...
def test001(desc="""purgedata table with one 2GB partition."""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist): return
    
    stmt = """Create Table """ + defs.my_schema + """.pd183
(
char0_n10           Character(2)
default 'AD' heading 'char0_n10 with default AD',
sbin0_uniq          int not null primary key,
...
attribute extent 4096, maxextents 512;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    
    ##sh import ${my_schema}.pd183 -I ${qagdata1}/b2pwl02.2gb
    stmt = gvars.inscmd + """ """ + defs.my_schema + """.pd183 select * from """ + gvars.g_schema_cmureg + """.b2pwl02;"""
    output = _dci.cmdexec(stmt)
    
    stmt = """select count(*) from """ + defs.my_schema + """.pd183;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_str_token(output, '2000000')
    
    # purgedata the table to other catalog.
    
    stmt = """purgedata """ + defs.my_schema + """.pd183;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    ...
    stmt = """drop table """ + defs.my_schema + """.pd183;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
    
    #                   End of test case pd183
