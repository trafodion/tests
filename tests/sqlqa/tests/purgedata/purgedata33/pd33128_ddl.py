...
def _init(hptestmgr, testlist=[]):
    ...
    if hpdci.tgtSQ(): 
        ...
        stmt = """control query default POS_NUM_OF_PARTNS '128';"""
        output = _dci.cmdexec(stmt)
    
    stmt = """create table pd33128
(
...
string4         char(52)        not null not droppable
    ...
    stmt = """create view pd33128v1 (v_unique3, v_ten) as
select unique3, ten from pd33128;"""
    output = _dci.cmdexec(stmt)
    
    stmt = """create index pd33128a1 on pd33128 (two , four);"""
    output = _dci.cmdexec(stmt)
