...
def _init(hptestmgr, testlist=[]):
    ...
    if hpdci.tgtSQ(): 
        ...
        stmt = """control query default POS_NUM_OF_PARTNS '16';"""
        output = _dci.cmdexec(stmt)
    
    stmt = """create table pd33016
(
...
string4         char(52)        not null not droppable
    ...
    stmt = """create view pd33016v1 (v_unique3, v_ten) as
select unique3, ten from pd33016;"""
    output = _dci.cmdexec(stmt)
    
    stmt = """create index pd33016a1 on pd33016 (two , four);"""
    output = _dci.cmdexec(stmt)
