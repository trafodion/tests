...
def _init(hptestmgr, testlist=[]):
    ...
    if hpdci.tgtSQ(): 
        ...
        stmt = """control query default POS_NUM_OF_PARTNS '64';"""
        output = _dci.cmdexec(stmt)
    
    stmt = """create table pd33064
(
...
string4         char(52)        not null not droppable
    ...
    stmt = """create view pd33064v1 (v_unique3, v_ten) as
select unique3, ten from pd33064;"""
    output = _dci.cmdexec(stmt)
    
    stmt = """create index pd33064a1 on pd33064 (two , four);"""
    output = _dci.cmdexec(stmt)
