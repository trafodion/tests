...
dim3.b = 4;"""
    ...
    ##expectstat $test_dir/A02opt5 P1
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
    
    ##expectfile $test_dir/A02opt5 P2
    stmt = """explain options 'f' xx;"""
    output = _dci.cmdexec(stmt)
    
    # NCI set envvar NO_SCREEN_OUTPUT;
    ##expectfile $test_dir/A02opt5 P3
    stmt = """execute xx;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output)
    # NCI reset envvar NO_SCREEN_OUTPUT;
    
    ##expectstat $test_dir/A02opt5 P4
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
...
dim2.b = 3;"""
    ...
    ##expectstat $test_dir/A03opt5 P1
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
    
    ##expectfile $test_dir/A03opt5 P2
    stmt = """explain options 'f' xx;"""
    output = _dci.cmdexec(stmt)
    
    # NCI set envvar NO_SCREEN_OUTPUT;
    ##expectfile $test_dir/A03opt5 P3
    stmt = """execute xx;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output)
    # NCI reset envvar NO_SCREEN_OUTPUT;
    
    ##expectstat $test_dir/A03opt5 P4
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
...
dim4.b < 2;"""
    ...
    ##expectstat $test_dir/A04opt5 P1
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
    
    ##expectfile $test_dir/A04opt5 P2
    stmt = """explain options 'f' xx;"""
    output = _dci.cmdexec(stmt)
    
    # NCI set envvar NO_SCREEN_OUTPUT;
    ##expectfile $test_dir/A04opt5 P3
    stmt = """execute xx;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output)
    # NCI reset envvar NO_SCREEN_OUTPUT;
    
    ##expectstat $test_dir/A04opt5 P4
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
...
fact.f < 2;"""
    ...
    ##expectstat $test_dir/A05opt5 P1
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
    
    ##expectfile $test_dir/A05opt5 P2
    stmt = """explain options 'f' xx;"""
    output = _dci.cmdexec(stmt)
    
    ##expectfile $test_dir/A05opt5 P3
    stmt = """execute xx;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output)
    
    ##expectstat $test_dir/A05opt5 P4
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
...
fact.f < 2;"""
    ...
    ##expectstat $test_dir/A06opt5 P1
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
    
    ##expectfile $test_dir/A06opt5 P2
    stmt = """explain options 'f' xx;"""
    output = _dci.cmdexec(stmt)
    
    ##expectfile $test_dir/A06opt5 P3
    stmt = """execute xx;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output)
    
    ##expectstat $test_dir/A06opt5 P4
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
...
dim4.b < 5;"""
    ...
    ##expectstat $test_dir/A07opt5 P1
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
    
    ##expectfile $test_dir/A07opt5 P2
    stmt = """explain options 'f' xx;"""
    output = _dci.cmdexec(stmt)
    
    # NCI set envvar NO_SCREEN_OUTPUT;
    ##expectfile $test_dir/A07opt5 P3
    stmt = """execute xx;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output)
    # NCI reset envvar NO_SCREEN_OUTPUT;
    
    ##expectstat $test_dir/A07opt5 P4
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
...
dim3.b < 3;"""
    ...
    ##expectstat $test_dir/A08opt5 P1
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
    
    ##expectfile $test_dir/A08opt5 P2
    stmt = """explain options 'f' xx;"""
    output = _dci.cmdexec(stmt)
    
    # NCI set envvar NO_SCREEN_OUTPUT;
    ##expectfile $test_dir/A08opt5 P3
    stmt = """execute xx;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output)
    # NCI reset envvar NO_SCREEN_OUTPUT;
    
    ##expectstat $test_dir/A08opt5 P4
    # NCI display statistics;
    stmt = """get statistics;"""
    output = _dci.cmdexec(stmt)
