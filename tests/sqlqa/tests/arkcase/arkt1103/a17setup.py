...
;"""
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss1')
    #
...
;"""
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss2')
    #
    # Include NULL value.
...
;"""
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss3')
    #
    # Include duplicate value.
...
;"""
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss4')
    #
    # Create the views.
    #
    # ======================================
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss5')
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss6')
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss7')
...
;"""
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss8')
    #
    # ======================================
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss9')
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss10')
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss11')
...
;"""
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss12')
    #
    # ======================================
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss13')
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss14')
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss15')
...
;"""
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss16')
    #
    # ======================================
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss17')
...
;"""
    ...
    #   create table tempvw4 (c1 varchar(7), c2 char(3), c3 varchar(7), c4 char(3),
    stmt = """c5 varchar(7), c6 char(3), c7 varchar(7), c8 char(3));"""
    output = _dci.cmdexec(stmt)
    
    ##expect any *--- SQL operation complete.*
    #   create view view4 as select * from tempvw4 for protection;
    ...
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss18')
    #
    # ======================================
    # (5) Instead of view5 as elsewhere make
    #     view6 with more (10) columns; and
    #     view7 with fewer (7) columns.
    #
    # Join on column values (including non=ajacent tables);
    #     aggregated results. All char.
    #     <view#> is <table1>
    #		LEFT JOIN <table2>
    #		LEFT JOIN <table3>
    #		LEFT JOIN <table4>
    ...
    stmt = """select max(t1.vch7), min(t1.chu3)
, max(t2.vch7), min(t2.chu3)
, max(t3.vch7), min(t3.chu3)
, 'Partway through'
, max(t4.vch7), min(t4.chu3)
, 'The end'
from tab1 t1
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss19')
    
    stmt = """create view view6 
( c1, c2, c3, c4, c5, c6, c7, c8, c9, c10) as
select max(t1.vch7), min(t1.chu3)
, max(t2.vch7), min(t2.chu3)
, max(t3.vch7), min(t3.chu3)
, 'Partway through'
, max(t4.vch7), min(t4.chu3)
...
;"""
    ...
    stmt = """select * from view6 ;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss20')
    # ======================================
...
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss21')
    
    stmt = """create view view7 
( c1, c2, c3, c4, c5, c6, c7) as
select max(t1.vch7), min(t1.chu3)
, max(t2.vch7), min(t2.chu3)
, max(t3.vch7)
, max(t4.vch7), min(t4.chu3)
...
;"""
    ...
    stmt = """select * from view7 ;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a17sexp""", 'a17ss22')
