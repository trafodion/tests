...
def test001(desc="""create tables"""):
    ...
    stmt = """create table TTREAD ( n1 integer
, c2 varchar(7)
, c3 char(5)
, c4 varchar(4)
) no partition;"""
...
, nsint smallint signed
    ...
    stmt = """insert into TTREAD values
( 1, 'a'  , NULL    , NULL )
, ( 2, 'cc' , 'cc'    , 'cc' )
, ( 3, 'cd' , 'alph'  , 'cc' )
, ( 4, 'c'  , 'e'     , 'c'  )
...
;"""
    output = _dci.cmdexec(stmt)
    # insert into TTF1 values
    #                     ('abcdefg',3,'cc',0.09,    'alph',2     ,'cc',1)
    #                   ;
    # insert into TTF2 select * from TTF ;
    
    stmt = """create table TAB1 (vc varchar(9) ) no partition ;"""
    output = _dci.cmdexec(stmt)
    stmt = """create table TAB4 (vc9 varchar(9) , vc7 varchar(7)
, vc5 varchar(5) , vc3 varchar(3)
) no partition ;"""
    output = _dci.cmdexec(stmt)
    
    stmt = """create table TNODEF1 (
vchar1 varchar(3) NO DEFAULT
, nint integer
) no partition ;"""
    output = _dci.cmdexec(stmt)
    stmt = """create table TNODEF5 (
vchar1 varchar(3) NO DEFAULT
, vc7 varchar(7)    NO DEFAULT
, vc5 varchar(5)    NO DEFAULT
, vc3 varchar(3)    NO DEFAULT
, nint integer      NO DEFAULT
) no partition ;"""
    output = _dci.cmdexec(stmt)
    
    # Table and updateable view -- audit.
    stmt = """create table BTAUD  (c1 varchar(51),c2 int) no partition ;"""
    output = _dci.cmdexec(stmt)
    
    # create view VUAUD as select * from BTAUD   ;
    stmt = """insert into BTAUD values ('abc',1), ('aud',2), ('abe',3);"""
    output = _dci.cmdexec(stmt)
    
    # Table and updateable view -- no audit.
    # create table BTNOAUD(c1 varchar(51),c2 int) attribute no audit;
    
    # create view VUNOAUD as select * from BTNOAUD   ;
    # insert into BTNOAUD values ('abd',2), ('noa',99);
