...
def test001(desc="""a01"""):
    ...
    #  Description:        This test verifies the SQL/K SELECT [first <N>]
    ...
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.btsel01 
where char_1 = 'A'
...
order by binary_signed;"""
    ...
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.btsel02 
where pic_x_1 > 'A'
order by syskey;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a01exp""", 'a01s5')
    
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.btsel03 
...
order by syskey;"""
    ...
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.btsel04 
where var_char < 'pam'
order by syskey;"""
    output = _dci.cmdexec(stmt)
...
order by syskey;"""
    ...
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.btsel05 
where pic_x_a > 'abe'
order by syskey;"""
    output = _dci.cmdexec(stmt)
...
order by pic_x_a, pic_x_b, pic_x_c;"""
    ...
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.btsel07 
where pic_x_b < 'Q'
order by pic_x_c, pic_x_b, pic_x_a;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a01exp""", 'a01s17')
    
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.btsel08 
...
order by large_int;"""
    ...
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.btsel09 
where pic_x_4 >= 'JOE'
...
order by pic_x_a desc, pic_x_6 desc;"""
    ...
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.btsel10 
where pic_x_a <> 'tom'
...
order by pic_9_7 asc, pic_x_7 desc, pic_x_a asc;"""
    ...
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.btsel11 
where pic_x_a > 'bob'
...
order by decimal_10 desc, pic_x_a asc, pic_9_7 desc;"""
    ...
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.pvsel01 
where binary_signed > 50
order by pic_decimal_3;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a01exp""", 'a01s28')
    
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.svsel11 
where col_1 > 50
...
order by syskey;"""
    ...
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.pvsel03 
where new_name_4 <> 0
order by new_name_4;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output, 2)
    
    stmt = """select [first 2] * from  """ + gvars.g_schema_arkcasedb + """.svsel13 
where new_name_4 <> 0
order by new_name_4;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output, 2)
...
def test002(desc="""a02"""):
    ...
    #         normal where clause select [first 4] statement - this tests the use
    #         of a where predicate where columns are compared.
    #
    
    stmt = """select [first 4] * from """ + gvars.g_schema_arkcasedb + """.btsel01 
...
order by binary_signed;"""
    ...
    stmt = """select [first 4] * from """ + gvars.g_schema_arkcasedb + """.btsel03 
...
order by syskey;"""
    ...
    stmt = """select [first 4] * from """ + gvars.g_schema_arkcasedb + """.btsel04 
...
order by syskey;"""
    ...
    stmt = """select [first 4] * from """ + gvars.g_schema_arkcasedb + """.btsel06 
where (col_1 < col_2) and
(col_3 <= col_4) and
(col_5 > col_6) and
(col_7 >= col_8) and
...
order by pic_x_a, pic_x_b, pic_x_c;"""
    ...
    stmt = """select [first 4] * from """ + gvars.g_schema_arkcasedb + """.btsel07 
where pic_x_a = pic_x_c
order by pic_x_c, pic_x_b, pic_x_a;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a02exp""", 'a02s19')
    
    stmt = """select [first 4] * from """ + gvars.g_schema_arkcasedb + """.btsel08 
...
order by large_int;"""
    ...
    stmt = """select [first 4] * from """ + gvars.g_schema_arkcasedb + """.btsel09 
...
order by pic_9_7 asc, pic_x_7 desc, pic_x_a asc;"""
    ...
    stmt = """select [first 4] * from """ + gvars.g_schema_arkcasedb + """.btsel11 
...
order by decimal_10 desc, pic_x_a asc, pic_9_7 desc;"""
    ...
    stmt = """select [first 4] * from """ + gvars.g_schema_arkcasedb + """.pvsel02 
where pic_x_1 = 'B'
order by pic_x_1;"""
    output = _dci.cmdexec(stmt)
...
def test004(desc="""a04"""):
    ...
    stmt = """select [first 3] * from """ + gvars.g_schema_arkcasedb + """.btsel01 
...
order by binary_signed;"""
    ...
    stmt = """select [first 3] * from """ + gvars.g_schema_arkcasedb + """.btsel02 
where pic_x_1 in ('B','C')
order by syskey;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a04exp""", 'a04s1')
    
    stmt = """select [first 3] * from """ + gvars.g_schema_arkcasedb + """.btsel03 where
...
order by decimal_10 desc, pic_x_a asc, pic_9_7 desc;"""
    ...
    stmt = """select [first 3] * from """ + gvars.g_schema_arkcasedb + """.pvsel04 
...
def test005(desc="""a05"""):
    ...
    stmt = """select [first 10] * from """ + gvars.g_schema_arkcasedb + """.btsel01 
...
order by binary_signed;"""
    ...
    stmt = """select [first 10] * from """ + gvars.g_schema_arkcasedb + """.btsel02 
where pic_x_1 like '_'
order by syskey;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a05exp""", 'a05s2')
    
    stmt = """select [first 10] * from """ + gvars.g_schema_arkcasedb + """.btsel03 
...
order by syskey;"""
    ...
    stmt = """select [first 10] * from """ + gvars.g_schema_arkcasedb + """.btsel04 
where var_char like 'b%'
order by syskey;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a05exp""", 'a05s5')
    
    stmt = """select [first 10] * from """ + gvars.g_schema_arkcasedb + """.btsel05 
where col_9 not like '__e'
order by syskey;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a05exp""", 'a05s6')
    
    stmt = """select [first 10] * from """ + gvars.g_schema_arkcasedb + """.btsel06 
where pic_x_a like '%'
order by pic_x_a, pic_x_b, pic_x_c;"""
    output = _dci.cmdexec(stmt)
...
order by pic_x_a, pic_x_b, pic_x_c;"""
    ...
    stmt = """select [first 10] * from """ + gvars.g_schema_arkcasedb + """.btsel07 
...
def test006(desc="""a06"""):
    ...
    #         select [first 1] normal test case with where clause - this tests
...
order by pic_comp_1;"""
    ...
    stmt = """select [first 3] * from """ + gvars.g_schema_arkcasedb + """.pvsel01 
where binary_signed > 50
for read committed access
order by pic_decimal_3;"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """select [first 3] * from """ + gvars.g_schema_arkcasedb + """.svsel11 
where col_1 > 50
for read committed access
order by col_4, col_8;"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """select [first 3] * from """ + gvars.g_schema_arkcasedb + """.pvsel02 
where pic_x_1 = 'B'
for serializable access
order by pic_x_1;"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """select [first 3] * from """ + gvars.g_schema_arkcasedb + """.svsel13 
where medium_int = 1000
for serializable access
order by new_name_4;"""
    output = _dci.cmdexec(stmt)
...
where new_name_1 = 80;"""
    ...
    #  select [first 1] aggregates on 0 records
...
where binary_32_u < 0;"""
    ...
    stmt = """select [first 1] avg(binary_64_s)
from """ + gvars.g_schema_arkcasedb + """.btsel03 
where binary_32_u < 0;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a08exp""", 'a08s4')
    
    stmt = """select [first 1] count(*), avg(binary_64_s)
from """ + gvars.g_schema_arkcasedb + """.btsel03 
where binary_32_u < 0;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a08exp""", 'a08s5')
    
    stmt = """select [first 1] count(*), avg(binary_64_s), avg(binary_32_u)
from """ + gvars.g_schema_arkcasedb + """.btsel03 
where binary_32_u < 0;"""
    output = _dci.cmdexec(stmt)
...
def test009(desc="""a09"""):
    ...
    stmt = """select [first 1] a.empnum, a.empname, ''
...
order by 1,2;"""
    ...
    stmt = """select [first 1] * from taba8a 
where a like '%n%';"""
    output = _dci.cmdexec(stmt)
...
where upshift(a) like upshift(?P5)  escape '\\';"""
    ...
    stmt = """select [first 1] * from taba8b 
where a like '%n%';"""
    output = _dci.cmdexec(stmt)
...
values ('B', 'B', 'B', 'B', 'B', 'B','B', 'B', 'B');"""
    ...
    stmt = """select [first 1] cs from t1 where pa like 'A%';"""
    output = _dci.cmdexec(stmt)
    ...
    #  two-column primary key.
    #  Prior to C30.S06 the following query returned zero rows.
    stmt = """select [first 1] cs from t1 where pa = 'B' and pb like 'B%';"""
    output = _dci.cmdexec(stmt)
    ...
    #  two-column index.
    #  Prior to C30.S06 the following query returned zero rows.
    stmt = """select [first 1] cs from t1 where i1a like 'A%';"""
    output = _dci.cmdexec(stmt)
...
primary key (c1));"""
    ...
    stmt = """select [first 2] c1 from t1 where c1 like '%A0%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a11exp""", 'a11s8')
    
    stmt = """select [first 2] c1 from t1 where c1 like 'A% ';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output, 0)
    
    #  LIKEs using a column that is not a key.
    
    stmt = """select [first 2] c2 from t1 where c2 like '%' order by c2;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a11exp""", 'a11s9')
    
    stmt = """select [first 2] c2 from t1 where c2 like '_';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output, 0)
    
    stmt = """select [first 2] c2 from t1 where c2 like '';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_selected_msg(output, 0)
    
    stmt = """select [first 2] c2 from t1 where c2 like 'A%0%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a11exp""", 'a11s10')
    
    stmt = """select [first 2] c2 from t1 where c2 like 'BB01%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a11exp""", 'a11s11')
    
    stmt = """select [first 2] c2 from t1 where c2 like 'CC0%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a11exp""", 'a11s12')
    
    stmt = """select [first 2] c2 from t1 where c2 like '_A0%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a11exp""", 'a11s13')
    
    stmt = """select [first 2] c2 from t1 where c2 like 'A%%';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a11exp""", 'a11s14')
    
    stmt = """select [first 2] c2 from t1 where c2 like '%A0%';"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """select [first 2] c1 from t1 where c1 like 'BB02 %';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/a11exp""", 'a11s16')
    
    stmt = """select [first 2] c2 from t1 where c2 like 'BB02 %';"""
    output = _dci.cmdexec(stmt)
...
FROM b2t1;"""
    ...
    stmt = """SELECT [first 1]  (int_col),   (largeint_col),   (real_col),   (double_col)
FROM b2t1;"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """SELECT [first 1] SUM (int_col)
FROM b2t1;"""
    output = _dci.cmdexec(stmt)
...
FROM b2t2;"""
    ...
    #
    #  Hard coded SELECT [first 1]s because PREPARE and EXECUTE above don't work.
    #
