...
def test001(desc="""create tables"""):
    ...
    # Adapted from arkt1199.testA05.
    # Based in part upon SQLDPOP table b2pwl02 but
    # column names that end with 'uniq' in b2pwl02 now end with 'u',
    # as values of these columns are no longer unique.
    stmt = """Create Table T5fixed 
(
char0_n10           Char(2) ,
...
;"""
    output = _dci.cmdexec(stmt)
    #
    # 3/13/96.  Added aggregates to view V5fixed;
...
;"""
    ...
    stmt = """create view  V5fixed ( n1, c2, c3, c4 ) as
select  max(t1.sbinneg15_nuniq) , min(t1.char2_2)
, min(t2.char2_2) , max(t3.char3_4)
from T5fixed t1
right join T5fixed t2 on t1.char2_2   = t2.char3_4
left  join T5fixed t3 on t2.char2_2   = t3.char3_4
group by t1.sbinneg15_nuniq , t1.char2_2 , t2.char2_2 , t3.char3_4
;"""
    output = _dci.cmdexec(stmt)
    
    stmt = """create table TAB1 ( vc varchar(10) ) no partition;"""
    output = _dci.cmdexec(stmt)
    
    stmt = """create table TAB4 ( vc9 varchar(10) DEFAULT '9 varchar'
