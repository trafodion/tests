...
def test_eqchar(desc="""equals & char"""):
    global _testmgr
    global _testlist
    global _dci
    if not _testmgr.testcase_begin(_testlist):
        return
    defs.testid = """HQC: equals & char"""
    ...
    output = _dci.cmdexec("""drop table F00CHAR cascade;""")

    stmt = """create table F00CHAR(colchar char(75) not null primary key)
salt using 8 partitions
;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)

    stmt = """load into F00CHAR select
...
;"""
    ...
    stmt = """update statistics for table F00CHAR on colchar;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)

    stmt = """showstats for table F00CHAR on colchar detail;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_complete_msg(output)
...
where interval_number in (53,54,60,61);"""
    ...
    ## ==================================
    ## EQUAL PREDICATE ON CHAR COLUMN
    ## ==================================
    # showstats for table F00CHAR on colchar detail;
    ...
    stmt = defs.prepXX + """select * from F00CHAR
where colchar = 'aaaaaaaaa';"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = defs.prepXX + """select * from F00CHAR
where colchar = 'zzzzzzzzz';"""
    output = _dci.cmdexec(stmt)
    _dci.expect_prepared_msg(output)
    output = _dci.cmdexec("""execute XX;""")
    _dci.expect_selected_msg(output, '0')
    defs.hkey = """SELECT * FROM F00CHAR WHERE COLCHAR = #NP# ;"""
    defs.num_hits = 1
...
where colchar = 'muHO6]789';"""
    ...
    if defs.genexp == 0:
        ...
        _dci.expect_file(output, defs.expfile, 'EQUAL_CHAR00A')
...
where colchar = 'btBN5]789';"""
    ...
    if defs.genexp == 0:
        output = _dci.cmdexec("""execute show_entries;""")
        _dci.expect_file(output, defs.expfile, 'EQUAL_CHAR00B')
...
where colchar = 'brEU@+789';"""
    ...
    if defs.genexp == 0:
        output = _dci.cmdexec("""execute show_entries;""")
        _dci.expect_file(output, defs.expfile, 'EQUAL_CHAR00C')
...
where colchar = 'dxAX8{789';"""
    ...
    if defs.genexp == 0:
        output = _dci.cmdexec("""execute show_entries;""")
        _dci.expect_file(output, defs.expfile, 'EQUAL_CHAR00D')
...
where colchar = 'mpBQ2>789';"""
    ...
    if defs.genexp == 0:
        output = _dci.cmdexec("""execute show_entries;""")
        _dci.expect_file(output, defs.expfile, 'EQUAL_CHAR00E')
...
where colchar = 'jyBT!]789';"""
    ...
    if defs.genexp == 0:
        output = _dci.cmdexec("""execute show_entries;""")
        _dci.expect_file(output, defs.expfile, 'EQUAL_CHAR00F')
