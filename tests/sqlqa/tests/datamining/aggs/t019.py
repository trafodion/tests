...
def test001(desc="""test019"""):
    ...
    # on a numeric (10, 5) column containing 100 rows of with mixed
...
from num100;"""
    ...
    _dci.expect_file(output, defs.test_dir + """/test019exp""", 'test019a')
...
from num100;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test019exp""", 'test019b')
    # expect 1 row with the following values:
...
from num100;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test019exp""", 'test019c')
    # expect 1 row with the following values:
    #
