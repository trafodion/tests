...
def test001(desc="""test012"""):
    ...
    # test012
    # Sequence function tests: RUNNINGCOUNT(column)
    #
    stmt = """select runningcount (i2) as runningcount_i2,
i1, i2, ts
from vwseqtb2 sequence by ts;"""
    ...
    # Equivalent standard SQL query:
    stmt = """select (select count(i2) from vwseqtb2 x where x.i1 <= y.i1)
as runningcount_i2,
i1, i2, ts
from vwseqtb2 y
order by ts;"""
