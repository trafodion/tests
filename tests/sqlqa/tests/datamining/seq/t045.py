...
def test001(desc="""test045"""):
    ...
    #
    stmt = """select a, c,
movingcount(c,5,150) as mcount_c,
movingsum(c,5,150) as msum_c
from vwseqtb43 
sequence by a;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test045.exp""", 's1')
    # expect movingcount to go from 1 - 5 and then all fives, dropping to
    #    fours on rows 53-57 and 85-89
    # expect movingsum at row 14 = 25210, 42 = 73443, 67 = 112080, 99 = 157408
    
    stmt = """select a, c,
movingmax(c,5,150) as mmax_c,
movingmin(c,5,150) as mmin_c
from vwseqtb43 
sequence by a;"""
    output = _dci.cmdexec(stmt)
    ...
    stmt = """select a, c,
movingsum(c,5,150) as msum_c,
cast(movingavg(c,5,150) as dec(18,3)) as mavg_c
from vwseqtb43 
sequence by a;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test045.exp""", 's3')
    # expect movingsum at row 14 = 25210, 42 = 74677, 66 = 111587, 99 = 157408
    #      & mvavg at row 14 = 5042, 39 = 13791, 66 = 22317.4, 99 = 31481.6
    
    stmt = """select a, c,
cast (movingstddev(c,5,150) as dec (18,3)) as mstdev_c,
cast (movingvariance(c,5,150) as dec (18,3)) as mvar_c
from vwseqtb43 
sequence by a;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test045.exp""", 's4')
