...
def test001(desc="""test088"""):
    ...
    # JClear
    # 1999-01-20
    # sequence functions: inner join : moving functions (2 args)
    #        movingcount
    #        movingsum
    #        movingmax
    #        movingmin
...
order by id;"""
    ...
    _dci.expect_file(output, defs.test_dir + """/test088.exp""", 's1')
    # expect 50 rows
...
order by seqtb81t1.id;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test088.exp""", 's2')
    # expect 20 rows
...
order by seqtb81t1.id;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test088.exp""", 's3')
    # expect 20 rows
