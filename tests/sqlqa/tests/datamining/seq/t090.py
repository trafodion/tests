...
def test001(desc="""test090"""):
    ...
    # JClear
    # 1999-01-20
    # sequence functions: cross join : running functions
    #        runningcount
    #        runningsum
    #        runningmax
    #        runningmin
...
order by lname;"""
    ...
    _dci.expect_file(output, defs.test_dir + """/test090.exp""", 's1')
...
order by seqtb81t1.id;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test090.exp""", 's2')
    # expect 20 rows
...
order by seqtb81t1.id;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test090.exp""", 's3')
    # expect 20 rows
