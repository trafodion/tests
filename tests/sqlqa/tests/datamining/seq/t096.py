...
def test001(desc="""test096"""):
    ...
    # JClear
    # 1999-01-20
    # sequence functions: plain join : running functions
    #        runningcount
    #        runningsum
    #        runningmax
    #        runningmin
...
order by id;"""
    ...
    _dci.expect_file(output, defs.test_dir + """/test096.exp""", 's1')
...
order by id;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test096.exp""", 's2')
    # expect 20 rows
...
order by id;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test096.exp""", 's3')
    # expect 20 rows
