...
def test001(desc="""test093"""):
    ...
    # JClear
    # 1999-01-20
    # sequence functions: join : running functions
    #        runningcount
    #        runningsum
    #        runningmax
    #        runningmin
...
order by id;"""
    ...
    _dci.expect_file(output, defs.test_dir + """/test093.exp""", 's1')
    # expect 47 rows
...
order by id;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test093.exp""", 's2')
    # expect 20 rows
...
order by id;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test093.exp""", 's3')
    # expect 20 rows
