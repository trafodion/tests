...
def test001(desc="""test006"""):
    ...
    #
    stmt = """select PROJ.pname, WORKS.hours
from PROJ, WORKS 
where WORKS.empnum in
(select STAFF.empnum
from STAFF,WORKS 
...
order by 1, 2;"""
    ...
    stmt = """select VWPROJ.pname, VWWORKS.hours
from VWPROJ, VWWORKS 
where VWWORKS.empnum in
(select VWSTAFF.empnum
from VWSTAFF,VWWORKS 
where VWSTAFF.empnum = VWWORKS.empnum
and VWWORKS.hours > 20)
order by 1, 2;"""
    output = _dci.cmdexec(stmt)
