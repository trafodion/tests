...
def test001(desc="""test007"""):
    ...
    # 1. First-N after the where clause
...
sort by a desc;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test007.exp""", 's3')
...
order by a desc;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test007.exp""", 's4')
...
sample first 10 rows;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test007.exp""", 's5')
...
sample first 10 rows;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_file(output, defs.test_dir + """/test007.exp""", 's6')
