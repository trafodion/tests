...
part
where
""" + defs.my_schema + """.qa_udf_integer(l_partkey) = """ + defs.my_schema + """.qa_udf_integer(p_partkey)
and l_shipdate >= date '1996-01-01'
and l_shipdate < date '1996-01-01' + interval '1' month;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_prepared_msg(output)
...
p_partkey = ps_partkey
and s_suppkey = ps_suppkey
and """ + defs.my_schema + """.qa_udf_integer(p_size) = 5 
and p_type like '%NICKEL'
and s_nationkey = n_nationkey
and n_regionkey = r_regionkey
and r_name = 'AMERICA'
...
where
p_partkey = ps_partkey
and """ + defs.my_schema + """.qa_udf_integer(s_suppkey) = ps_suppkey
and s_nationkey = n_nationkey
and n_regionkey = r_regionkey
and r_name = 'AMERICA'
)
...
and p_partkey = l_partkey
and o_orderkey = l_orderkey
and """ + defs.my_schema + """.qa_udf_integer(s_nationkey) = """ + defs.my_schema + """.qa_udf_integer(n_nationkey)
and p_name like '%maroon%'
) as profit
group by
nation,
...
where
l_shipdate >= date '1997-01-01'
and """ + defs.my_schema + """.qa_udf_date(l_shipdate) < date '1997-01-01' + interval '1' year
and l_discount between 0.03 - 0.01 and 0.03 + 0.01
and l_quantity < 25;"""
    output = _dci.cmdexec(stmt)
    _dci.expect_prepared_msg(output)
...
and r_name = 'MIDDLE EAST'
and s_nationkey = n2.n_nationkey
and """ + defs.my_schema + """.qa_udf_date(o_orderdate) between date '1995-01-01'  and date '1996-12-31'
and p_type = 'LARGE ANODIZED COPPER'
) as all_nations
group by
o_year
...
(
select
""" + defs.my_schema + """.qa_udf_integer(""" + defs.my_schema + """.qa_udf_integer(c_custkey)),
count(o_orderkey)
from
customer left outer join orders on
c_custkey = o_custkey
...
and c_custkey = o_custkey
and l_orderkey = o_orderkey
and """ + defs.my_schema + """.qa_udf_date(o_orderdate) < date '1995-03-12'
and """ + defs.my_schema + """.qa_udf_date(l_shipdate) > date '1995-03-12'
group by
l_orderkey,
o_orderdate,
o_shippriority
...
and p_brand <> 'Brand#13'
and p_type not like 'SMALL PLATED%'
and """ + defs.my_schema + """.qa_udf_integer(p_size) in (""" + defs.my_schema + """.qa_udf_integer(22), """ + defs.my_schema + """.qa_udf_integer(14), """ + defs.my_schema + """.qa_udf_integer(8), """ + defs.my_schema + """.qa_udf_integer(18), """ + defs.my_schema + """.qa_udf_integer(49), """ + defs.my_schema + """.qa_udf_integer(1), """ + defs.my_schema + """.qa_udf_integer(33), """ + defs.my_schema + """.qa_udf_integer(6))
and ps_suppkey not in (
select
s_suppkey
from
...
where
l_orderkey = o_orderkey
and """ + defs.my_schema + """.qa_udf_date(l_commitdate) < """ + defs.my_schema + """.qa_udf_date(l_receiptdate)
)
group by
o_orderpriority
order by
...
lineitem
where
""" + defs.my_schema + """.qa_udf_date(l_shipdate) >= """ + defs.my_schema + """.qa_udf_date(date '1994-02-01')
and l_shipdate < date '1994-02-01' + interval '3' month
group by
l_suppkey
) as v(supplier_no, total_revenue)
...
lineitem
where
""" + defs.my_schema + """.qa_udf_date(l_shipdate) <= date '1998-12-01' - interval '69' day (3)
group by
l_returnflag,
l_linestatus
order by
...
and o_orderdate < date '1994-07-01' + interval '3' month
and l_returnflag = 'R'
and """ + defs.my_schema + """.qa_udf_integer(c_nationkey) = """ + defs.my_schema + """.qa_udf_integer(n_nationkey)
group by
c_custkey,
c_name,
c_acctbal,
...
and p_brand = 'Brand#22'
and p_container in ('MED BAG', 'MED BOX', 'MED PKG', 'MED PACK')
and """ + defs.my_schema + """.qa_udf_integer(""" + defs.my_schema + """.qa_udf_integer(l_quantity)) >= 13 and l_quantity <= 13 + 10
and p_size between 1 and 10
and l_shipmode in ('AIR', 'AIR REG')
and l_shipinstruct = 'DELIVER IN PERSON'
)
...
and n_regionkey = r_regionkey
and r_name = 'ASIA'
and """ + defs.my_schema + """.qa_udf_date(o_orderdate) >= date '1997-01-01'
and o_orderdate < date '1997-01-01' + interval '1' year
group by
n_name
order by
...
and """ + defs.my_schema + """.qa_udf_date(l_shipdate) between date '1995-01-01' and date '1996-12-31'
) as shipping
group by
supp_nation,
cust_nation,
...
and l_shipmode in ('REG AIR', 'AIR')
and l_commitdate < l_receiptdate
and """ + defs.my_schema + """.qa_udf_date(l_shipdate) < """ + defs.my_schema + """.qa_udf_date(l_commitdate)
and l_receiptdate >= date '1995-01-01'
and l_receiptdate < date '1995-01-01' + interval '1' year
group by
l_shipmode
