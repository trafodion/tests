...
int main (int argc, char *argv[])
{
        ...
        printf("Drop table if it exists...\n");
        //Drop table LargeColSmDataTbl if it exists
        returnCode = SQLExecDirect(hstmt, (SQLCHAR*)"DROP TABLE ODBC32K.LargeColSmDataTbl", SQL_NTS);
        if(!SQL_SUCCEEDED(returnCode))
                LogDiagnostics("SQLExecDirect of DROP", returnCode);

        printf("Creating ODBC32K table LargeColSmDataTbl...\n");
        ...
        //SELECT * from LargeColSmDataTbl 
        printf("Fetching data using SQLExecDirect, SQLFetch, SQLGetData\n");
        returnCode = SQLExecDirect(hstmt, (SQLCHAR*)"SELECT * from ODBC32K.LargeColSmDataTbl", SQL_NTS);
        if(!SQL_SUCCEEDED(returnCode))
        ...
        while (TRUE)
        {
                ...
                {
                        SQLGetData(hstmt, 1, SQL_C_CHAR, strC1, 200, &cbC1);
                        SQLGetData(hstmt, 2, SQL_C_CHAR, strC2, 200, &cbC2);
