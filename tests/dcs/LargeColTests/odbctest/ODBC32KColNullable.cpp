...
int main (int argc, char *argv[])
{
        ...
        printf("Drop sampleNull table if it exists...\n");
        //Drop the sampleNull table LargeColTbl if it exists
        returnCode = SQLExecDirect(hstmt, (SQLCHAR*)"DROP TABLE sampleNull.LargeColTbl", SQL_NTS);
        if(!SQL_SUCCEEDED(returnCode))
                LogDiagnostics("SQLExecDirect of DROP", returnCode);

        printf("Creating sampleNull table LargeColTbl...\n");
        //CREATE TABLE LargeColTbl (ID INT NOT NULL, TASK VARCHAR(10), LAST_UPDATE TIMESTAMP, PRIMARY KEY (C1));
        returnCode = SQLExecDirect(hstmt, (SQLCHAR*)"CREATE SCHEMA sampleNull", SQL_NTS);        
        if(!SQL_SUCCEEDED(returnCode))
                LogDiagnostics("SQLExecDirect of CREATE", returnCode);
        returnCode = SQLExecDirect(hstmt, (SQLCHAR*)"CREATE TABLE sampleNull.LargeColTbl ( C1 varchar (200000), C2 CHAR(200000))", SQL_NTS);
        if(!SQL_SUCCEEDED(returnCode))
                LogDiagnostics("SQLExecDirect of CREATE", returnCode);
        printf("Table LargeColTbl created using SQLExecDirect.\n");
        ...
        SQLCHAR strC1[200001], strC2[200001];
        ...
        returnCode = SQLPrepare(hstmt, (SQLCHAR*)"INSERT INTO sampleNull.LargeColTbl VALUES (?, ?)", SQL_NTS);
        if(!SQL_SUCCEEDED(returnCode))
        ...
        strcat ((char*)strC1, "EE");
        
        strcpy ((char*)strC2, "S");
        for (int i = 0; i < 199996; i++) {
             strcat ((char*)strC2, "b");
        }
        strcat ((char*)strC2, "EE");
        ...
        //SELECT * from LargeColTbl 
        printf("Fetching data using SQLExecDirect, SQLFetch, SQLGetData\n");
        returnCode = SQLExecDirect(hstmt, (SQLCHAR*)"SELECT * from sampleNull.LargeColTbl", SQL_NTS);
        if(!SQL_SUCCEEDED(returnCode))
        ...
        while (TRUE)
        {
                ...
                {
                        SQLGetData(hstmt, 1, SQL_C_CHAR, strC1, 200001, &cbC1);
                        SQLGetData(hstmt, 2, SQL_C_CHAR, strC2, 200001, &cbC2);
                        printf("Data selected: %s %s\n", strC1, strC2);
                }
                else
                        break;
        }
