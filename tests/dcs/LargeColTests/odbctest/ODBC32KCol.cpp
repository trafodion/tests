...
int main (int argc, char *argv[])
{
        ...
        //Drop the sample table LargeColTbl if it exists
        returnCode = SQLExecDirect(hstmt, (SQLCHAR*)"DROP TABLE sample.LargeColTbl", SQL_NTS);
        if(!SQL_SUCCEEDED(returnCode))
                LogDiagnostics("SQLExecDirect of DROP", returnCode);

        printf("Creating sample table LargeColTbl...\n");
        //CREATE TABLE LargeColTbl (ID INT NOT NULL, TASK VARCHAR(10), LAST_UPDATE TIMESTAMP, PRIMARY KEY (C1));
        returnCode = SQLExecDirect(hstmt, (SQLCHAR*)"CREATE SCHEMA sample", SQL_NTS);        
        if(!SQL_SUCCEEDED(returnCode))
                LogDiagnostics("SQLExecDirect of CREATE", returnCode);
        returnCode = SQLExecDirect(hstmt, (SQLCHAR*)"CREATE TABLE sample.LargeColTbl ( C1 varchar (200000) NOT NULL, C2 CHAR(200000))", SQL_NTS);
        if(!SQL_SUCCEEDED(returnCode))
                LogDiagnostics("SQLExecDirect of CREATE", returnCode);
        ...
        returnCode = SQLPrepare(hstmt, (SQLCHAR*)"INSERT INTO sample.LargeColTbl VALUES (?, ?)", SQL_NTS);
        if(!SQL_SUCCEEDED(returnCode))
        ...
        returnCode = SQLExecDirect(hstmt, (SQLCHAR*)"SELECT * from sample.LargeColTbl", SQL_NTS);
        if(!SQL_SUCCEEDED(returnCode))
