...
/* end of sq new */
	...
	char dbms_name[]={"Trafodion Database"};
	char dbms_ver[]={"02.03.0054"};
...
#ifdef unixcli
	char driver_name[]={"libtrafodbc64"};
#else
	char driver_name[]={"trfodbc1.DLL"};
#endif /* WIN32 */
	char driver_odbc_ver[]={"03.51"}; // must be >= 3.51
	char driver_ver[]={"02.03"};
	...
	char user_name[32]; // size is based upon SQL_MAX_USER_NAME_LEN
...
//=====================Modified for Longvarchar Changes========================================================
	LogMsg(LINEBEFORE+SHORTTIMESTAMP,"Begin testing API => MX specific SQLGetInfo | SQLGetInfo | getinfo.c\n");
	...
	for (i = 0; i < TotalOptions; i++)
	{
		switch (ExpectedResults[i].InfoType)
		{
			...
			case SQL_SERVER_NAME:
				strncpy(server_name,pTestInfo->Server+4,3);
				server_name[3] = '\0';
				strupr(server_name);
