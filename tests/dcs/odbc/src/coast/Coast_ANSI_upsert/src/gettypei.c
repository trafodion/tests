...
{   
	...
	struct
	{
		...
		{	//TYPENAME,		DATATYPE,		PREC,LITPRE,LITSUF,	PARAM,			NULL,			CASE,SRCH,				ATTR,MON,INC,LOC,		MIN,MAX
			...
			{"TIMESTAMP",	SQL_TIMESTAMP,	26, "{ts '","'}",	"",				SQL_NULLABLE,	0,	SQL_ALL_EXCEPT_LIKE,0,	0,	0,	"TIMESTAMP",0,	6},
...
//================Modified for Longvarchar Changes===========================================================
	...
	LogMsg(LINEBEFORE+SHORTTIMESTAMP,"Begin testing API =>SQLGetTypeInfo | SQLGetTypeInfo | gettypei.c\n");
...
//=========================================================================================
	...
	while (TypeInfo[j].DATA_TYPE != END_LOOP) 
	{
		...
		LogMsg(NONE,"SQLGetTypeInfo(hstmt,%d\n", TypeInfo[j].DATA_TYPE);
		...
		else
		{
			...
			{ 
				...
				//if(!pTestInfo->bLongOn && _stricmp(TypeInfo[j].TYPE_NAME,"LONG VARCHAR"))
				//{
