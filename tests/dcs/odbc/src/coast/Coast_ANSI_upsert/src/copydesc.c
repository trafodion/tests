...
typedef struct {
	SQLINTEGER    sPartID;
	SQLLEN        cbPartID; //  
	SQLCHAR		  szDescription[DESC_LEN];
...
//===========================================================================================================
	var_list_t *var_list;
	var_list = load_api_vars("SQLCopyDescriptor", charset_file);
	if (var_list == NULL) return FAILED;

	Data[0].DrpTab		= var_mapping("SQLCopyDescriptor_Data_DrpTab_0", var_list);
	Data[0].CrtTab		= var_mapping("SQLCopyDescriptor_Data_CrtTab_0", var_list);
	Data[0].InsTab[0]	= var_mapping("SQLCopyDescriptor_Data_InsTab0_0", var_list);
	Data[0].InsTab[1]	= var_mapping("SQLCopyDescriptor_Data_InsTab1_0", var_list);
	Data[0].InsTab[2]	= var_mapping("SQLCopyDescriptor_Data_InsTab2_0", var_list);
	Data[0].SelTab		= var_mapping("SQLCopyDescriptor_Data_SelTab_0", var_list);

	Data[1].DrpTab		= var_mapping("SQLCopyDescriptor_Data_DrpTab_1", var_list);
	Data[1].CrtTab		= var_mapping("SQLCopyDescriptor_Data_CrtTab_1", var_list);
	Data[1].InsTab[0]	= var_mapping("SQLCopyDescriptor_Data_InsTab0_1", var_list);
//========================================================================================================

	LogMsg(LINEBEFORE+SHORTTIMESTAMP,"Begin testing API => SQLCopyDesc | SQLCopyDescriptor | copydesc.c\n");
	TEST_INIT;
   ...
   if (returncode != SQL_SUCCESS)
   {
	...
	SQLExecDirect(hstmt0,(SQLCHAR *)Data[0].DrpTab, SQL_NTS);

	SQLExecDirect(hstmt1,(SQLCHAR *)Data[1].DrpTab, SQL_NTS);

	i = sizeof(rget)/sizeof(rget[0]);
	for(j=0; j<i; j++) {
		rget[j].sPartID = i;
		strcpy((char*)(rget[j].szDescription),"abc123");
		rget[j].sPrice = 0.0;
	}
...
// APD and IPD of hstmt1
	...
	returncode = SQLExecDirect(hstmt0,(SQLCHAR *)Data[0].CrtTab, SQL_NTS);
	...
	returncode = SQLExecDirect(hstmt0,(SQLCHAR *)Data[0].InsTab[0], SQL_NTS);
	...
	returncode = SQLExecDirect(hstmt0,(SQLCHAR *)Data[0].InsTab[1], SQL_NTS);
	...
	returncode = SQLExecDirect(hstmt0,(SQLCHAR *)Data[0].InsTab[2], SQL_NTS);
	...
	returncode = SQLExecDirect(hstmt0, (SQLCHAR *)Data[0].SelTab, SQL_NTS);
	...
	returncode = SQLBindCol(hstmt0, 2, SQL_C_CHAR, &rget[0].szDescription, DESC_LEN, &rget[0].cbDescription);
	if(!CHECKRC(SQL_SUCCESS,returncode,"SQLBindCol"))
	{
		LogAllErrorsVer3(henv,hdbc,hstmt0);
		FullDisconnect(pTestInfo);
	...
	LogMsg(NONE,"%s\n",Data[1].CrtTab);
	returncode = SQLExecDirect(hstmt1,(SQLCHAR *)Data[1].CrtTab, SQL_NTS);
	...
	LogMsg(NONE,"%s\n",Data[1].InsTab[0]);
	returncode = SQLPrepare(hstmt1, (SQLCHAR *)Data[1].InsTab[0], SQL_NTS);
...
//========================================================================================================
	...
	LogMsg(SHORTTIMESTAMP+LINEAFTER,"End testing API => SQLCopyDesc.\n");
