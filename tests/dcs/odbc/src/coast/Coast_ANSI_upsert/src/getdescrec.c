...
/* *******************************************************************************************
	filenmame: SetGetDescRec.c
	This file contains test for the ODBC 3.0 apis SQLGetDescRec and SQLSetDescRec
*******************************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <windows.h>
...
//local defines
#define DATA_ARR_LEN	6	// number of elements in data arrays
...
{   
	...
	//	DescName		Type				DatetimeIntervalCode	DescOctetLength		Precision	DescScale	DescDataPtr		OctectLengthPtr		IndicatorPtr				DescNullable
		{"",			SQL_C_DEFAULT,				0,						0,				0,			0,			NULL,			0,				SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_C_CHAR,					0,						80,				0,			0,			NULL,			0,				SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_C_CHAR,					0,						300,			0,			0,			NULL,		SQL_NTS,			SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_C_NUMERIC,				0,						0,				0,			0,			NULL,		SQL_DATA_AT_EXEC,	SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_C_TYPE_TIMESTAMP,	SQL_CODE_TIMESTAMP,			0,				6,			1,			NULL,			0,				SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_C_INTERVAL_SECOND,	SQL_CODE_SECOND,			0,				2,			0,			NULL,			0,				SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN }
	};
	...
	//IPD
	//	DescName		Type				DatetimeIntervalCode	DescOctetLength		Precision	DescScale	DescDataPtr		OctectLengthPtr		IndicatorPtr				DescNullable
		{"",			SQL_C_DEFAULT,				0,						0,				0,			0,			0,				0,					0,					SQL_NULLABLE_UNKNOWN },
		{"",			SQL_CHAR,					0,						80,				0,			0,			NULL,			0,				SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_CHAR,					0,						300,			0,			0,			NULL,		SQL_NTS,			SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_NUMERIC,				0,						0,				0,			0,			NULL,		SQL_DATA_AT_EXEC,	SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_TYPE_TIMESTAMP,		SQL_CODE_TIMESTAMP,			0,				6,			1,			0,				0,				SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_INTERVAL_SECOND,	SQL_CODE_SECOND,			0,				2,			0,			NULL,			0,				SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN }
	};
	...
	//	DescName		Type				DatetimeIntervalCode	DescOctetLength		Precision	DescScale	DescDataPtr		OctectLengthPtr		IndicatorPtr				DescNullable
		...
		{"",			SQL_C_CHAR,					0,						300,			0,			0,			NULL,		SQL_NTS,			SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_C_NUMERIC,				0,						0,				0,			0,			NULL,		SQL_DATA_AT_EXEC,	SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_C_TIMESTAMP,		SQL_CODE_TIMESTAMP,			0,				6,			1,			NULL,			0,				SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN },
		{"",			SQL_C_INTERVAL_SECOND,	SQL_CODE_SECOND,			0,				2,			0,			NULL,			0,				SQL_NULL_DATA,			SQL_NULLABLE_UNKNOWN }
	};
...
//==================================================================================================================
	LogMsg(LINEBEFORE+SHORTTIMESTAMP,"Begin testing API => SQLSetDescRec and SQLGetDescRec | SQLGetDescRec | getdescrec.c\n");
	TEST_INIT;
...
//===============================================================================================================	
	...
	{
		LogAllErrorsVer3(henv,hdbc,hstmt);
		LogMsg(ERRMSG,"Cannot allocate descriptor: Aborting Implicit APD tests.\n");
		TEST_FAILED;
	...
	for (i = 0;i < DATA_ARR_LEN; i++)		// i = index to TestDataAPD
	{
		...
		/*LogMsg(NONE,"SQLSetDescRec(handle,%d,%d,%d,%d,%d,%d,%s,%d,%d)\n",
											setDescRecNumber, 
											setDescType, 
											setDescIntervalCode, 
											setDescOctetLength , 
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLSetDescRec: positive tests for Implicit APD: TestDataAPD['%d'].\n",i);
			//FullDisconnect(pTestInfo);
			TEST_FAILED;
			//TEST_RETURN;
			continue;
		...
		LogMsg(NONE,"SQLSetDescRec / SQLGetDescRec: positive tests for Implicit APD.\n");


		getDescRecHandle		= hImApd;					
		getDescRecNumber		= RecNumber;				
		getNameMaxLength		= MAXLEN;
		strcpy(getDescName	,"");	//initialize
		setDescNullable			= TestDataAPD[i].DescNullable;

		returncode = SQLGetDescRec(getDescRecHandle, getDescRecNumber, (SQLCHAR*)getDescName, getNameMaxLength, &getDescNameActualLength, &getDescType, &getDescIntervalCode, &getDescOctectLength, &getDescPrecision, &getDescScale, &getDescNullable);
		if(!CHECKRC(SQL_SUCCESS,returncode,"SQLGetDescRec"))
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLGetDescRec: positive tests for Implicit APD: TestDataAPD['%d'].\n",i);
			//FullDisconnect(pTestInfo);
			TEST_FAILED;
			//TEST_RETURN;
			continue;
		...
		//check  SQL_DESC_NAME
		if (!(strcmp(getDescName, setDescName)))
		{
			LogMsg(NONE,"SQL_DESC_NAME value returned is '%s'\n",getDescName);
		}
		else
		{
			TEST_FAILED;					
			LogMsg(ERRMSG, "expect SQL_DESC_NAME: '%s' and actual: '%s' are not matched\n", setDescName,getDescName);
		}
		
		//check SQL_DESC_TYPE
		if (getDescType == setDescType)
		{
			LogMsg(NONE,"SQL_DESC_TYPE value returned is '%d'\n",getDescType);
		}
		else
		{
			TEST_FAILED;					
			LogMsg(ERRMSG, "expect SQL_DESC_TYPE: '%d' and actual: '%d' are not matched\n", setDescType, getDescType);
		}

		//check SQL_DESC_DATETIME_INTERVAL_CODE
		if (getDescIntervalCode == setDescIntervalCode)
		{
			LogMsg(NONE,"SQL_DESC_DATETIME_INTERVAL_CODE value returned is '%d'\n",getDescIntervalCode);
		}
		else
		{
			TEST_FAILED;					
			LogMsg(ERRMSG, "expect SQL_DESC_DATETIME_INTERVAL_CODE: '%d' and actual: '%d' are not matched\n", setDescIntervalCode, getDescIntervalCode);
		}
		//check SQL_DESC_OCTECT_LENGTH
		if (getDescOctectLength == setDescOctetLength)
		{
			LogMsg(NONE,"SQL_DESC_OCTECT_LENGTH value returned is '%d'\n",getDescOctectLength);
		}
		else
		{
			TEST_FAILED;					
			LogMsg(ERRMSG, "expect SQL_DESC_OCTECT_LENGTH: '%d' and actual: '%d' are not matched\n", setDescOctetLength,getDescOctectLength);
		}

		//check SQL_DESC_PRECISION
		if (getDescPrecision == setDescPrecision)
		{
			LogMsg(NONE,"SQL_DESC_PRECISION value returned is '%d'\n",getDescPrecision);
		}
		else
		{
			TEST_FAILED;					
			LogMsg(ERRMSG, "expect SQL_DESC_PRECISION: '%d' and actual: '%d' are not matched\n", setDescPrecision,getDescPrecision);
		}

		//check SQL_DESC_SCALE
		if (getDescScale == setDescScale)
		{
			LogMsg(NONE,"SQL_DESC_SCALE value returned is '%d'\n",getDescScale);
		}
		else
		{
			TEST_FAILED;					
			LogMsg(ERRMSG, "expect SQL_DESC_SCALE: '%d' and actual: '%d' are not matched\n", setDescScale,getDescScale);
		}

		//check SQL_DESCNULLABLE
		if (getDescNullable == setDescNullable)
		{
			LogMsg(NONE,"SQL_DESC_NULLABLE value returned is '%d'\n",getDescNullable);
		}
		else
		{
			TEST_FAILED;					
			LogMsg(ERRMSG, "expect SQL_DESC_NULLABLE: '%d' and actual: '%d' are not matched\n", setDescNullable, getDescNullable);
		}
		// end of checks 
		TESTCASE_END;
...
//===============================================================================================================
	...
	{
		LogAllErrorsVer3(henv,hdbc,hstmt);
		LogMsg(ERRMSG,"Cannot allocate descriptor: Aborting Explicit APD tests.\n");
		TEST_FAILED;
		//TEST_RETURN;
	...
	for (i = 0;i < DATA_ARR_LEN; i++)		// i = index to TestDataAPD
	{
		...
		/*LogMsg(NONE,"SQLSetDescRect(handle,%d,%d,%d,%d,%d,%d,%s,%d,%d)\n",
									setDescRecNumber, 
									setDescType, 
									setDescIntervalCode, 
									setDescOctetLength , 
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLSetDescRec: positive tests for Explicit APD: TestDataAPD['%d'].\n",i);
			TEST_FAILED;
			//TEST_RETURN;
			continue;
		}
		...
		//LogMsg(SHORTTIMESTAMP,"SQLGetDescRec: test for Implicit APD.\n");
		LogMsg(NONE,"SQLSetDescRec / SQLGetDescRec: positive tests for Explicit APD.\n");
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLGetDescRec: positive tests for Explicit APD: TestDataAPD['%d'].\n",i);
			TEST_FAILED;
			continue;
		}
...
//===============================================================================================================
	...
	{
		LogAllErrorsVer3(henv,hdbc,hstmt);
		LogMsg(ERRMSG,"Cannot free explicit descriptor: Aborting Explicit-Implicit Descriptor tests.\n");
		TEST_FAILED;
		goto ARDtests;		// transfers control to the next set of tests
	}
	...
	getDescRecHandle		= hImApd;
		...
		//Get the Implicit APD Descriptor 
		LogMsg(NONE,"SQLGetDescRec: tests for Explicit-Implicit Descriptor.\n");
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLGetDescRec: tests for Explicit-Implicit APD: TestDataAPD['%d'].\n",i);
			TEST_FAILED;
			//continue;
		}
...
//===================================================================================================================
	...
	{
		LogAllErrorsVer3(henv,hdbc,hstmt);
		LogMsg(ERRMSG, "Cannot get ARD: Aborting Implicit ARD tests.\n");
		TEST_FAILED;
	...
	for (i = 0;i < DATA_ARR_LEN; i++)		// i = index to TestDataARD
	{
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLSetDescRec: positive tests for Implicit ARD: TestDataARD['%d'].\n",i);
			//FullDisconnect(pTestInfo);
			TEST_FAILED;
			//TEST_RETURN;
			continue;
		...
		LogMsg(NONE,"SQLSetDescRec / SQLGetDescRec: positive tests for Implicit ARD.\n");
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLGetDescRec: positive tests for Implicit ARD: TestDataARD['%d'].\n",i);
			//FullDisconnect(pTestInfo);
			TEST_FAILED;
			//TEST_RETURN;
			break;
...
//===================================================================================================================
	...
	for (i = 0;i < DATA_ARR_LEN; i++)		// i = index to TestDataARD
	{
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLSetDescRec: positive tests for Explicit ARD: TestDataARD['%d'].\n",i);
			TEST_FAILED;
			continue;
		}
		...
		LogMsg(NONE,"SQLSetDescRec and SQLGetDescRec: positive tests for Explicit ARD.\n");
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLGetDescRec: positive tests for Explicit ARD: TestDataARD['%d'].\n",i);
			TEST_FAILED;
			break;
		}
...
//=====================================================================================================================	
	...
	{
		LogAllErrorsVer3(henv,hdbc,hstmt);
		LogMsg(ERRMSG, "Cannot get IPD: Aborting Implicit IPD tests.\n");
		TEST_FAILED;
	...
	for (i = 0;i < DATA_ARR_LEN; i++)		// i = index to TestDataIPD
	{
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLSetDescRec: positive tests for Implicit IPD: TestDataIPD['%d'].\n",i);
			//FullDisconnect(pTestInfo);
			TEST_FAILED;
			//TEST_RETURN;
			continue;
		...
		LogMsg(NONE,"SQLSetDescRec and SQLGetDescRec: positive tests for Implicit IPD.\n");
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG,"SQLGetDescRec: positive tests for Implicit IPD: TestDataIPD['%d'].\n",i);
			//FullDisconnect(pTestInfo);
			TEST_FAILED;
			//TEST_RETURN;
			continue;
...
//==================================================================================================================
		...
		{
			LogAllErrorsVer3(henv,hdbc,hstmt);
			LogMsg(ERRMSG, "Cannot get IRD: Aborting Implicit IRD tests.\n");
			TEST_FAILED;
		...
		returncode = SQLGetDescRec(hImIrd, 1, (SQLCHAR*)getDescName, MAXLEN, &getDescNameActualLength, &getDescType, &getDescIntervalCode, &getDescOctectLength, &getDescPrecision, &getDescScale, &getDescNullable);
...
//===============================================================================================================
	...
	if(!CHECKRC(SQL_SUCCESS,returncode,"SQLAllocHandle"))
	{
		...
		LogMsg(ERRMSG,"Cannot allocate statement: Aborting populated Implicit IRD tests.\n");
		FullDisconnect(pTestInfo);
		TEST_FAILED;
	...
	{
		LogAllErrorsVer3(henv,hdbc,hstmt1);
		LogMsg(ERRMSG,"Cannot allocate descriptor: Aborting populated Implicit IRD tests.\n");
		TEST_FAILED;
		goto Cleanup;		// transfers control to the next set 
	}
	...
	{
		LogAllErrorsVer3(henv,hdbc,hstmt1);
		LogMsg(ERRMSG,"Cannot execute statement: Aborting populated Implicit IRD tests.\n");
		TEST_FAILED;
		goto Cleanup;		// transfers control to the next set 
	}	

	//check IRD
	returncode = SQLGetDescRec(hIrd, 1, (SQLCHAR*)getDescName, MAXLEN, &getDescNameActualLength, &getDescType, &getDescIntervalCode, &getDescOctectLength, &getDescPrecision, &getDescScale, &getDescNullable);
...
Cleanup: ;
	...
	LogMsg(SHORTTIMESTAMP+LINEAFTER,"End testing API => SQLSetDescRec and SQLGetDescRec.\n");
    TEST_RETURN;
} //end of test ****************
