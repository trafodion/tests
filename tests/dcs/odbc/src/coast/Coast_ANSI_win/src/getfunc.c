...
{   
	...
	LogMsg(LINEBEFORE+SHORTTIMESTAMP,"Begin testing API =>SQLGetFunctions | SQLGetFunctions | getfunc.c\n");
  ...
  if (pTestInfo->hdbc == (SQLHANDLE)NULL)
	...
	for (fFunction = 1; fFunction <= MAX_FUNC; fFunction++) /* fFunction = 0 will get all functions */
	{
		...
		{
			sprintf(Heading,"Test basic positive functionality of SQLGetFunctions for Function: %d\n",fFunction);
	...
	for (fFunction = 1; fFunction <= MAX_FUNC; fFunction++) /* fFunction = 0 will get all functions */
	{
		if (fFunction < 24 || fFunction > 39)
		{
			sprintf(Heading,"Test Positive functionality of SQLGetFunctions for Function: %d\n"
			                "with output as NULL\n",fFunction);
			TESTCASE_BEGIN(Heading);
			if (fFunction == 0)
				returncode = SQLGetFunctions((SQLHANDLE)hdbc,fFunction,fALLExists);
	...
	for (fFunction = 1; fFunction <= MAX_FUNC; fFunction++) /* fFunction = 0 will get all functions */
	{
		if (fFunction < 24 || fFunction > 39)
		{
			sprintf(Heading,"Test negative functionality of SQLGetFunctions for Function: %d\n"
			                "with hdbc as NULL\n",fFunction);
			TESTCASE_BEGIN(Heading);
			if (fFunction == 0)
       		returncode = SQLGetFunctions((SQLHANDLE)hdbc,fFunction,fALLExists);
	...
	LogMsg(SHORTTIMESTAMP+LINEAFTER,"End testing API => SQLGetFunctions.\n");
	TEST_RETURN;
}
