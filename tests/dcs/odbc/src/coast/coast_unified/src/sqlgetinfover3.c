...
//	TCHAR  *dbms_ver=_T("01.03.0000");
	...
	//TCHAR  *dm_ver=_T("03.51.4202.0000"); //must be greatr than 3.51.0: checking in test code
...
//		{SQL_DM_VER, dm_ver, _tcslen(dm_ver), 1},
...
//		{SQL_ODBC_VER,odbc_ver,_tcslen(odbc_ver), 1},
	...
	for (i = 0; i < TotalOptions; i++)
	{
		switch (ExpectedResults[i].InfoType)
		{
			...
			case SQL_SERVER_NAME:
				_tcscpy(server_name,_T("TCP:xxx.xxx.xxx.xxx/xxxxx")); // starts with TCP:
				_tcsupr(server_name);
...
//					case SQL_KEYWORDS:
					...
					case SQL_DRIVER_ODBC_VER:
					case SQL_DM_VER:
					//	if(_tcsstr(u1.charInfoValue,_T("3.5"))==NULL)
