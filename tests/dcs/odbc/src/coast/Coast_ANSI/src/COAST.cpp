...
bool m_ExecDirect_LargeCol              = FALSE;
...
int Run20Tests()
{
	...
	if (m_ExecDirect)			
	{
        ...
        if (m_ExecDirect_LargeCol)
        {
                puts("Running ExecDirect API with large column table.\r\n");
                start = time( NULL);
                TestSQLExecDirect_LargeCol(pTestInfo);
                end = time( NULL);
                LogMsg(NONE,"Time elapsed: %f\n", difftime(end, start));
                LogResultsTest("SQLExecDirect_LargeCol (2.0)");
        }
	if (m_ExtendedFetch)				
	{
	...
	while (ret != NULL)
	{
		...
		if (_stricmp(token,"SQLAllocEnv") == 0)
			...
			if (_stricmp(token,"YES") == 0)
                ...
                else if (_stricmp(token,"SQLExecuteDirect_LargeCol") == 0)
                        {
                        token = strtok(NULL, seps);
                        if (_stricmp(token,"YES") == 0)
                                m_ExecDirect_LargeCol = TRUE;
                        else
                                m_ExecDirect_LargeCol = FALSE;
                        }
