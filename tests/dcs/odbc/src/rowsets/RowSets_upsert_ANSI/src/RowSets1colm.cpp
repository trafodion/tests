...
int main( int argc, char* argv[] )
{
    ...
    // REGULAR
    strcpy( ExecSQLStr[ 0 ],  "DROP TABLE ROWSET_T1 CASCADE" );
    strcpy (ExecSQLStr [1], "create table rowset_t1 (c1 time)"); 
    strcpy (ExecSQLStr [2], "upsert into rowset_t1 values (time '03:04:05')"); 
    strcpy( ExecSQLStr[ 3 ],  "SELECT * FROM rowset_t1" );
    strcpy( ExecSQLStr[ 4 ],  "DELETE FROM rowset_t1" );
    strcpy( ExecSQLStr[ 5 ],  "DELETE FROM rowset_t1 WHERE CURRENT OF TABLECURSOR" );
    strcpy( ExecSQLStr[ 6 ],  "DELETE FROM rowset_t1 WHERE C01 = ? " );
    strcpy( ExecSQLStr[ 7 ],  "UPDATE rowset_t1 SET C01=? WHERE C1 = ?" );
...
bool RowsetDML( )
{
	...
	if (rowsetRetcode != SQL_SUCCESS) {puts("before error");
	...
	{
               printf("retcodes rowsetRetcode=%d expectedRowsetRetcode=%d",rowsetRetcode,expectedRowsetRetcode);
